...
let a0=nfa_construct ([], [(1,100)], 
            [(0,[(1,48)],1);], [0], [1]);;

let f n e = match e with None -> None | Some c -> Some [(Z.of_int ((Z.to_int c) + n), Z.of_int ((Z.to_int c) + n))] ;;

let ff n f e = match e with [] -> Some e | (a, b) :: l -> Some [(Z.of_int (Z.to_int a + n), Z.of_int (Z.to_int b + n))];;

let fe f e = false;;

let tf n e = f n;;

let n = nft_construct ([], [(1,100)], [(0,((Some [(1,48)], 1),1))], [0], [1] , tf 4);;

let ne = product_transducer n a0 (ff 4) fe;;

let n1 = nfa_elim ne;;

let n2 = nfa_normal n1 ;;

print_auto (nfa_destruct n2);;

(*print_auto (nfa_destruct (nfa_tran_complement a2 (Automata_lib.Nat (Z.of_int 9)))) ;;


print_string "--------------------------\n" ;;

print_auto (nfa_destruct a2);;
*)

(* print_auto (nfa_destruct (nfa_complement (nfa_determinise (nfa_uniq (nfa_tran_complement a0 (Automata_lib.Nat (Z.of_int 9))))))) ;; *)
