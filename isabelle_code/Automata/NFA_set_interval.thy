...
imports Main LTS_set "../General/Accessible"
        ...
        "implementation/Interval_imp" NFA_states
begin
...
lemma NFA_construct_ba_alt_def :
  "NFA_construct_ba  sem (Q, D, I, F) =
   \<lparr> \<Q>=set Q \<union> set I \<union> set F \<union>
       ...
       \<Delta> = {(q1,l,q2). \<exists> l1. (q1,l1,q2) \<in> set D \<and> l = sem l1}, 
...
next
  ...
  have fold_lemma1: "NFA_construct_ba sem (Q, (qlq # D), I, F)= 
         construct_NFA_ba_aux sem (NFA_construct_ba sem (Q, D, I, F)) qlq"
...
proof -
  ...
  { fix q \<sigma> q'
    assume "(q, \<sigma>, q') \<in> \<Delta> (NFA_construct_ba sem (Q, D, I, F))"
...
proof -
  show "q \<in> \<I> \<A> \<Longrightarrow> q \<notin> NFA_unreachable_states \<A>"  
    using NFA_unreachable_states_def by force
qed
...
next
  ...
  from invar obtain s where 
    ...
    by (smt Collect_cong Un_insert_left 
            Un_insert_right case_prodD case_prodD' list.simps(15))
...
qed 
  ...
  have conclude1: "NFA_construct_reachable_abstract_impl_invar I FP D
    ...
    by (smt Collect_cong prop1 prop2)
...
assumes fin_S: "finite S"
  ...
  unfolding NFA_construct_reachable_abstract2_impl_def 
            NFA_construct_reachable_abstract_impl_def S_def[symmetric]
apply refine_rcg
apply (simp)
...
definition NFA_normalise_states :: "('q, 'a) NFA_rec \<Rightarrow> 
  ...
  "NFA_normalise_states \<A> = 
       (NFA_rename_states \<A> (SOME (f::'q \<Rightarrow> 'q2). inj_on f (\<Q> \<A>)))"
...
proof -
   ...
   have \<I>_prod_wf: "\<I> \<A>1 \<times> \<I> \<A>2 = \<I> (bool_comb_NFA bc \<A>1 \<A>2)"
     by simp
   have FP_wf: "(\<And>q. q \<in> \<Q> (bool_comb_NFA bc \<A>1 \<A>2) \<Longrightarrow>
        (case q of
         (q1, q2) \<Rightarrow> (q1, q2) \<in> \<Q> \<A>1 \<times> \<Q> \<A>2 \<and> bc (q1 \<in> \<F> \<A>1) (q2 \<in> \<F> \<A>2)) =
        (q \<in> \<F> (bool_comb_NFA bc \<A>1 \<A>2)))"
     by auto

   have \<Delta>_wf: "(LTS_product (\<Delta> \<A>1) (\<Delta> \<A>2)) = \<Delta> (bool_comb_NFA bc \<A>1 \<A>2)"
     by auto

   from NFA.NFA_remove_unreachable_states_implementation
        [of "bool_comb_NFA bc \<A>1 \<A>2" "\<I> \<A>1 \<times> \<I> \<A>2" 
             "\<lambda>(q1,q2). (q1, q2) \<in> (\<Q> \<A>1 \<times> \<Q> \<A>2) \<and> 
...
proof {
  assume accept_union: "NFA_accept
     \<lparr>\<Q> = \<Q> \<A>1 \<union> \<Q> \<A>2, \<Delta> = \<Delta> \<A>1 \<union> \<Delta> \<A>2, \<I> = \<I> \<A>1 \<union> \<I> \<A>2,
        \<F> = \<F> \<A>1 \<union> \<F> \<A>2\<rparr>
     w"
  ...
  obtain q q' where qq'_def: "q \<in>  \<I> \<A>1 \<union> \<I> \<A>2 \<and> q' \<in> \<F> \<A>1 \<union> \<F> \<A>2 \<and> 
    ...
    by (metis NFA_rec.select_convs(2) NFA_rec.select_convs(3) NFA_rec.select_convs(4))
  ...
  {
    let ?A = "\<lparr>\<Q> = \<Q> \<A>1 \<union> \<Q> \<A>2, \<Delta> = \<Delta> \<A>1 \<union> \<Delta> \<A>2, \<I> = \<I> \<A>1 \<union> \<I> \<A>2,
        \<F> = \<F> \<A>1 \<union> \<F> \<A>2\<rparr>"
...
proof -
  ...
  f_def: "inj_on f (\<Q> \<A>1) \<and>
       \<A>1' =
       \<lparr>\<Q> = f ` \<Q> \<A>1, \<Delta> = {(f s1, a, f s2) |s1 a s2. (s1, a, s2) \<in> \<Delta> \<A>1},
  ...
  have c2: "\<Q> \<A>1' \<union> \<Q> \<A>2' = ?f ` (\<Q> \<A>1 \<union> \<Q> \<A>2)"
    by auto
  ...
  have c4: "\<I> \<A>1' \<union> \<I> \<A>2' = ?f ` (\<I> \<A>1 \<union> \<I> \<A>2) \<and> \<F> \<A>1' \<union> \<F> \<A>2' = ?f ` (\<F> \<A>1 \<union> \<F> \<A>2)
         \<and>
    NFA \<lparr>\<Q> = \<Q> \<A>1 \<union> \<Q> \<A>2, \<Delta> = \<Delta> \<A>1 \<union> \<Delta> \<A>2, \<I> = \<I> \<A>1 \<union> \<I> \<A>2,
           \<F> = \<F> \<A>1 \<union> \<F> \<A>2\<rparr>"
    apply (rule_tac conjI)
