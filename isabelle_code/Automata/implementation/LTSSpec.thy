...
(* type_synonym ('V,'W,'L) lts_add = "'V \<Rightarrow>'W set \<Rightarrow> 'V \<Rightarrow> 'L \<Rightarrow> 'L" *)
type_synonym 'L lts_union = "'L \<Rightarrow> 'L \<Rightarrow> 'L"

  locale lts_union = lts +
    constrains \<alpha> :: "('V,'W,'L) lts_\<alpha>"
    fixes union :: "'L lts_union"
    assumes lts_union_correct:
      "invar l1 \<and> invar l2 \<Longrightarrow> invar (union l1 l2)"
      "invar l1 \<and> invar l2 \<Longrightarrow> \<alpha> (union l1 l2) = (\<alpha> l1) \<union> (\<alpha> l2)"
...
locale lts_succ_it' = lts +
    ...
    fixes succ_it :: "('V,'W,'\<sigma>,'L) lts_succ_it'"
    assumes lts_succ_it_correct:
      "invar l \<Longrightarrow> set_iterator (succ_it l v e) {v' | v'. (v, e, v') \<in> (\<alpha> l)}"
...
*)
  ...
  begin
    ...
    abbreviation union where "union \<equiv> clts_op_union ops"
