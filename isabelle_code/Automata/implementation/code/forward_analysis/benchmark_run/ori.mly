...
%token Lleft LLleft Lright LRright LReject LEOF Lcomm LBleft LBright
%token LAccept LArrow Lsep Lsepa Lassign Leq
%token <int list>Lstr
%token <string> Lvar
...
strCon:
	Lvar Lassign Lconcat LLleft Lvar Lcomm Lvar LRright	Lsepa   {Strconcat ($1, $5, $7)}
      | Lvar Leq Lstr Lsepa {Regexcon ($1, consAuto $3)}
      |	Lvar Lin automaton Lsepa {Regexcon ($1, $3)} 
...
automaton:
         LBleft initstate transitions LBright {{init=$2; trans=$3}}
      |  LBleft Lsingle LBright {consAuto (str_code_l1 0 $2)}
