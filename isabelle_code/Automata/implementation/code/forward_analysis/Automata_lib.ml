...
let rec rs_forward_analysis (_A1, _A2) _B _C f
  rm = (fun b si rc rma ->
         whilea
...
let rec rs_compute_ready_set _A
  si rc sr =
    iteratei_set_op_list_it_rs_ops _A si (fun _ -> true)
      (fun x sigma ->
        (let xa =
           (if not (is_none (lookup _A rc x))
             then iteratei_set_op_list_it_rs_ops (linorder_prod _A _A)
                    (the (lookup_aux _A x rc)) (fun _ -> true)
                    (fun xa sigmaa ->
                      ins_rm_basic_ops _A (fst xa)
                        (ins_rm_basic_ops _A (snd xa) sigmaa))
                    (empty_rm_basic_ops _A ())
             else empty_rm_basic_ops _A ())
           in
          (if g_subset_dflt_basic_oops_rm_basic_ops _A xa sr
            then ins_rm_basic_ops _A x sigma else sigma)))
      (empty_rm_basic_ops _A ());;
