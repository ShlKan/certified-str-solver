...
let compute_ready_set s rc r = Automata_lib.rs_compute_ready_set linorder_nat s rc r;;
...
let rec check_unsat_rm rm = 
      match (rm) with 
              []  -> "inconclusive" | 
    (v, a) :: rml -> (let (_,(l,(_,ac))) = nfa_destruct a in ((if (ac = []) 
                     then "unsat"                          
                     else check_unsat_rm rml))) ;;

(* 
let rec check_unsat_rm1 rm = 
      match (rm) with 
              []  -> "inclusive" | 
    (v, a) :: rml -> (let (_,(l,(_,ac))) = nfa_destruct a in (print_int (List.length l); print_string ";" ;print_newline(); if
                      ac = [] then (check_unsat_rm rml; "unsat") else check_unsat_rm rml)) ;;
*)
