...
type_synonym ('q,'a,'nfa) nfa_\<alpha> = "'nfa \<Rightarrow> ('q, 'a) NFA_rec"
  ...
  type_synonym ('q,'a,'nfa) nfa_from_list_interval = 
    "('q list \<times> ('a \<times> 'a) \<times> ('q \<times> ('a \<times> 'a) \<times> 'q) list 
  ...
  begin
    lemma nfa_from_list_interval_correct___isomorphic :
      "invar (from_interval l)"
      "(\<forall> (q,d,q') \<in> set (fst (snd (snd l))). finite (semI d)) \<longrightarrow>
       (\<forall> (q, \<sigma>, q') \<in> set (fst (snd (snd l))). semI \<sigma> \<subseteq> semI (fst (snd l))) \<longrightarrow>
...
locale nfa_rename_labels_gen = n1: nfa \<alpha>1 invar1 + 
  ...
  type_synonym ('q2,'i,'a,'\<sigma>,'nfa) nfa_construct = 
    "('q2 \<Rightarrow> 'i) \<Rightarrow> ('a \<times> 'a) \<Rightarrow> 'q2 list \<Rightarrow>  ('q2 \<Rightarrow> bool) \<Rightarrow> 
               ...
               (\<forall>f. inj_on f (\<Q> \<A>) \<longrightarrow>
                    (\<forall>ff. (\<forall>q. q \<in> \<Q> \<A> \<longrightarrow> ff q = f q) \<longrightarrow>
                          (\<forall>I. distinct I \<longrightarrow>
                               ...
                               (\<forall>a b. \<Sigma> \<A> = semI (a, b) \<longrightarrow>
...
(\<forall>D_it.
    ...
    invar (construct ff (a, b) I FP D_it) \<and>
    NFA_isomorphic_wf (\<alpha> (construct ff (a, b) I FP D_it))
     (NFA_remove_unreachable_states \<A>)))))))))"
                        ...
                        (\<forall>a b. \<Sigma> \<A> = semI (a, b) \<longrightarrow>
                               ...
                               (\<forall>FP. (\<forall>q. q \<in> \<Q> \<A> \<longrightarrow> FP q = (q \<in> \<F> \<A>)) \<longrightarrow>
                                     (\<forall>D_it.
                                         ...
                                         invar (construct f (a, b) I FP D_it) \<and>
                                         NFA_isomorphic_wf (\<alpha> (construct f (a, b) I FP D_it))
                                          (NFA_remove_unreachable_states \<A>)))))))"
      apply (rule impI allI)+
    proof -
      fix f I D a b FP D_it 
      show "NFA \<A> \<Longrightarrow>
       inj_on f (\<Q> \<A>) \<Longrightarrow>
       distinct I \<Longrightarrow>
       set I = \<I> \<A> \<Longrightarrow>
       finite D \<Longrightarrow>
       \<Sigma> \<A> = semI (a, b) \<Longrightarrow>
       \<forall>q. inj_on (\<lambda>(a, y). (semI a, y)) {(a, q'). (q, a, q') \<in> D} \<Longrightarrow>
       {(q, semI (a, b), q') |q a b q'. (q, (a, b), q') \<in> D} = \<Delta> \<A> \<Longrightarrow>
       ...
       invar (construct f (a, b) I FP D_it) \<and>
       NFA_isomorphic_wf (\<alpha> (construct f (a, b) I FP D_it)) (NFA_remove_unreachable_states \<A>)"
      proof -
      assume p2: "NFA \<A>" and
             p3: "inj_on f (\<Q> \<A>)" and
             p4: "distinct I" and
   ...
   NFA_isomorphic_wf (\<alpha> (construct f (a, b) I FP D_it))
    (NFA_remove_unreachable_states \<A>)"
      ...
      proof 
        ...
        qed qed
        from this cc1
  show "invar (construct f (a, b) I FP D_it) \<and>
    NFA_isomorphic_wf (\<alpha> (construct f (a, b) I FP D_it)) (NFA_remove_unreachable_states \<A>)"
    by simp
qed qed qed
   ...
   (\<forall>D_it.
         ...
         (\<forall>f. inj_on f (\<Q> \<A>) \<longrightarrow>
              (\<forall>I. distinct I \<longrightarrow>
                   ...
                   (\<forall>D. finite D \<longrightarrow>
                        (\<forall>a b. \<Sigma> \<A> = semI (a, b) \<longrightarrow>
                               (\<forall>q a b aa ba.
                                   (\<exists>q'. (q, ((a, b), aa, ba), q') \<in> D) \<longrightarrow> a \<le> b \<and> aa \<le> ba) \<longrightarrow>
    ...
    proof -
      ...
      show "NFA \<A> \<Longrightarrow>
       ...
       {(q, semI (a, b) \<inter> semI (aa, ba), q') |q a b aa ba q'. (q, ((a, b), aa, ba), q') \<in> D} =
       \<Delta> \<A> \<Longrightarrow>
       \<forall>q. q \<in> \<Q> \<A> \<longrightarrow> FP q = (q \<in> \<F> \<A>) \<Longrightarrow>
       \<forall>q. set_iterator (D_it q) {(a, q'). (q, a, q') \<in> D} \<Longrightarrow>
       invar (construct f (a, b) I FP D_it) \<and>
...
from p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 this suc1 show "invar (construct f (a,b) I FP D_it) \<and>
    NFA_isomorphic_wf (\<alpha> (construct f (a,b) I FP D_it)) (NFA_remove_unreachable_states \<A>)"
    by simp
qed qed qed
