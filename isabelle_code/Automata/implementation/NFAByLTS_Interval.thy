...
definition nfa_states :: "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> 'q_set" where
  "nfa_states A = fst A"
lemma [simp]: "nfa_states (Q, Sig, D, I, F) = Q" by (simp add: nfa_states_def)

definition nfa_alphabet :: "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> 'a \<times> 'a" where
...
definition nfa_trans :: 
        "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> 'd" where
  "nfa_trans A = (fst (snd (snd A)))"
lemma [simp]: "nfa_trans (Q, Sig, D, I, F) = D" by (simp add: nfa_trans_def)

definition nfa_initial :: "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> 'q_set" where
  "nfa_initial A = fst (snd (snd (snd  A)))"
lemma [simp]: "nfa_initial (Q, Sig, D, I, F) = I" by (simp add: nfa_initial_def)

definition nfa_accepting :: "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> 'q_set" where
...
definition nfa_statep :: "'qq_set \<times> ('a \<times> 'a) \<times> 'dd_nfa \<times> 'qq_set \<times> 'qq_set \<Rightarrow> 'qq_set" where
  "nfa_statep A = fst A"
lemma [simp]: "nfa_statep (Q, Sig, D, I, F) = Q" by (simp add: nfa_statep_def)

definition nfa_alphap :: "'qq_set \<times> ('a \<times> 'a) \<times> 'dd_nfa \<times> 'qq_set \<times> 'qq_set \<Rightarrow> 'a \<times> 'a" where
  "nfa_alphap A = fst (snd A)"
lemma [simp]: "nfa_alphap (Q, Sig, D, I, F) = Sig" by (simp add: nfa_alphap_def)

definition nfa_transp :: 
        "'qq_set \<times> ('a \<times> 'a) \<times> 'dd_nfa \<times> 'qq_set \<times> 'qq_set \<Rightarrow> 'dd_nfa" where
  "nfa_transp A = (fst (snd (snd A)))"
lemma [simp]: "nfa_transp (Q, Sig, D, I, F) = D" by (simp add: nfa_transp_def)

definition nfa_initialp :: "'qq_set \<times> ('a \<times> 'a) \<times> 'dd_nfa \<times> 'qq_set \<times> 'qq_set \<Rightarrow> 'qq_set" where
  "nfa_initialp A = fst (snd (snd (snd  A)))"
lemma [simp]: "nfa_initialp (Q, Sig, D, I, F) = I" by (simp add: nfa_initialp_def)

definition nfa_acceptingp :: "'qq_set \<times> ('a \<times> 'a) \<times> 'dd_nfa \<times> 'qq_set \<times> 'qq_set \<Rightarrow> 'qq_set" where
...
definition nfa_invar :: "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> bool" where
  "nfa_invar A =
...
definition nfa_\<alpha> :: "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> ('q, 'a) NFA_rec" 
  where
  "nfa_\<alpha> A =
   \<lparr> \<Q> = s.\<alpha> (nfa_states A),   
     \<Sigma> = semI (nfa_alphabet A),
...
definition nfa_invar_NFA :: "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> bool" where
...
definition nfa_construct_interval_aux ::
  "'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set \<Rightarrow> 'q \<times> ('a \<times> 'a) \<times> 'q \<Rightarrow> 
   'q_set \<times> ('a \<times> 'a) \<times> 'd \<times> 'q_set \<times> 'q_set" where 
   "nfa_construct_interval_aux = (\<lambda>(Q, Sig, D, I, F) (q1, l, q2).
    (s.ins q1 (s.ins q2 Q), Sig, 
     d.add q1 (intersectI l Sig) q2 D,
     I, F))"
...
proof -
  ...
  have DL_OK : "d.invar DL \<Longrightarrow> 
    ...
    show "nfa_\<alpha> (nfa_construct_interval_aux A (q1, l, q2)) = 
      ...
      apply (simp add: Interval.inter_correct lts_add.lts_add_correct(2))
...
fixes ll :: "'q list \<times> ('a ::linorder \<times> 'a) \<times> ('q \<times> ('a \<times> 'a) \<times> 'q) list \<times> 'q list \<times> 'q list"
...
proof -
  ...
  have A_\<alpha>: "nfa_\<alpha> ?A = \<lparr>\<Q> = set (QL @ IL @ FL), \<Sigma> = semI SigL, \<Delta> = {}, \<I> = set IL, \<F> = set FL\<rparr>"
...
proof -
   ...
   show ?thesis
     ...
     using nfa_invar_NFA_def NFA_construct_interval___is_well_formed
...
(* preprocess goals
  ...
  apply (rename_tac x1 x2 x3 x4 qm'' q' As n'' \<A>  qm n 
                    Qs bg ed DD Is Fs q2 q1 bga qm' n' D' bja r)
  apply (clarify, simp add: br_def DS_OK R'_def)
  defer
  apply (rename_tac y As x q rm \<A> qm n Qs bg ed DD Is Fs r)
using [[goals_limit = 6]]
proof -
  
  fix q rm \<A> qm n Qs bg ed DD Is Fs r
  {
   assume rm_q: "state_map_\<alpha> (qm, n) (q2_\<alpha> q) = Some r" and
         in_R': "rm = state_map_\<alpha> (qm, n) \<and> state_map_invar (qm, n)" and
         in_R: "\<A> = nfa_\<alpha> (Qs, (bg, ed), DD, Is, Fs) \<and> nfa_invar (Qs, (bg, ed), DD, Is, Fs)" and
  ...
  with in_R show "s.memb (the (qm.lookup (ff q) qm)) Qs = 
                (r \<in> \<Q> (nfa_\<alpha> (Qs, (bg, ed), DD, Is, Fs)))"
    unfolding R_def 
    by (simp add: nfa_invar_def s.correct nfa_\<alpha>_def)
}
  {
  fix x3 x4 qm'' q' \<A> qm n Qs bg ed DD Is Fs bga qm' n' D' bja r
  assume r_nin_Q: "r \<notin> \<Q> \<A>" and 
       ...
       state_map_invar (qm', n')" and
       p1: "d.invar D'" and
       in_R: "((Qs, (bg, ed), DD, Is, Fs), \<A>) \<in> R" and
  ...
  have "((s.ins_dj (the (qm.lookup (ff q') qm)) Qs, (bg, ed), D', Is,
  ...
  show "(FFP q' \<longrightarrow>
        (FP (q2_\<alpha> q') \<longrightarrow>
         ((s.ins_dj (the (qm.lookup (ff q') qm)) Qs, (bg, ed), D', Is,
       ...
       (\<not> FFP q' \<longrightarrow>
        (FP (q2_\<alpha> q') \<longrightarrow>
         ((s.ins_dj (the (qm.lookup (ff q') qm)) Qs, (bg, ed), D', Is, Fs),
          \<lparr>\<Q> = insert r (\<Q> \<A>), \<Sigma> = Sig, \<Delta> = interval_to_set ` d.\<alpha> D', \<I> = \<I> \<A>,
             \<F> = insert r (\<F> \<A>)\<rparr>)
         \<in> R) \<and>
        (\<not> FP (q2_\<alpha> q') \<longrightarrow>
...
assumes d_add_OK: 
  (* "\<forall>q a q' q''. (q, a, q') \<in> \<Delta> \<A> \<and> (q, a, q'') \<in> \<Delta> \<A> \<and> q'' \<noteq> q' \<longrightarrow> *)
    ...
    and wf_\<A>: "NFA \<A>"
    and sig_\<Sigma>: "\<Sigma> \<A> = semI Sig"
shows "nfa_invar_NFA (NFA_construct_reachable_interval_impl_code qm_ops ff Sig II FP D_it) \<and>
...
(* goal solved *)
  ...
  apply (rename_tac q' qm'' rm \<A> qm n Qs be ed DD Is Fs n'' q bga qm' n' D' bja r)
  apply (simp add: br_def)
  defer
  apply (rename_tac As q rm \<A> qm n Qs be ed DD Is Fs r)
using [[goals_limit = 6]]
proof -
  fix x y q rm \<A> qm n Qs be ed As DD Is Fs r
  {
   assume rm_q: "state_map_\<alpha> (qm, n) (q2_\<alpha> q) = Some r" and
         in_R': "rm = state_map_\<alpha> (qm, n) \<and> state_map_invar (qm, n)" and
         in_R: "\<A> = nfa_\<alpha> (Qs, (be, ed), DD, Is, Fs) \<and> nfa_invar (Qs, (be, ed), DD, Is, Fs)" and
         invar_q: "q2_invar q" and
  ...
  with in_R show "s.memb (the (qm.lookup (ff q) qm)) 
                Qs = (r \<in> \<Q> (nfa_\<alpha> (Qs, (be, ed), DD, Is, Fs)))"
    unfolding R_def by (simp add: nfa_invar_def s.correct nfa_\<alpha>_def)
}

  {
  fix x1 x2 x1b x2a x2b q qm'' rm \<A> qm n Qs be ed DD Is Fs n'' q' bga qm' n' D' bja r
    assume r_nin_Q: "r \<notin> \<Q> \<A>" and 
       rm_q': "state_map_\<alpha> (qm, n) (q2_\<alpha> q') = Some r" and
       weak_invar: "NFA_construct_reachable_abstract_impl_weak_invar 
             Sig I FP {(q, a1 \<inter> a2, q') |q a1 a2 q'. (q, (a1, a2), q') \<in> D}
       ...
       state_map_invar (qm, n) \<and> ((Qs, (be, ed), DD, Is, Fs), \<A>) \<in> R" and
       invar_q': "q2_invar q'" and 
  ...
  have semIA: "semI (be, ed) = \<Sigma> \<A>"
    apply simp
    using nfa_\<alpha>_def[of "(Qs, (be, ed), DD, Is, Fs)"]
    by simp
  ...
  have "((s.ins_dj (the (qm.lookup (ff q') qm)) Qs, (be, ed), D', Is,
         if FFP q' then s.ins_dj (the (qm.lookup (ff q') qm)) 
          (snd (snd (snd ((Qs, DD, Is, Fs))))) else 
           (snd (snd (snd ((Qs, DD, Is, Fs)))))),
        \<lparr>\<Q> = insert r (\<Q> \<A>), \<Sigma> = Sig, \<Delta> = (interval_to_set ` (d.\<alpha> D')), \<I> = \<I> \<A>,
  ...
  from this show "(FFP q' \<longrightarrow>
        (FP (q2_\<alpha> q') \<longrightarrow>
         ((s.ins_dj (the (qm.lookup (ff q') qm)) Qs, (be, ed), D', Is,
           s.ins_dj (the (qm.lookup (ff q') qm)) Fs),
          \<lparr>\<Q> = insert r (\<Q> \<A>), \<Sigma> = Sig, \<Delta> = interval_to_set ` d.\<alpha> D', \<I> = \<I> \<A>,
             \<F> = insert r (\<F> \<A>)\<rparr>)
         \<in> R) \<and>
       ...
       (\<not> FFP q' \<longrightarrow>
        (FP (q2_\<alpha> q') \<longrightarrow>
         ((s.ins_dj (the (qm.lookup (ff q') qm)) Qs, (be, ed), D', Is, Fs),
          \<lparr>\<Q> = insert r (\<Q> \<A>), \<Sigma> = Sig, \<Delta> = interval_to_set ` d.\<alpha> D', \<I> = \<I> \<A>,
             \<F> = insert r (\<F> \<A>)\<rparr>)
         \<in> R) \<and>
        (\<not> FP (q2_\<alpha> q') \<longrightarrow>
...
assumes d_add_OK: 
  (* "\<forall>q a q' q''. (q, a, q') \<in> \<Delta> \<A> \<and> (q, a, q'') \<in> \<Delta> \<A> \<and> q'' \<noteq> q' \<longrightarrow> *)
    ...
    and FP_OK: "\<And>q. q2_invar q \<Longrightarrow> q2_\<alpha> q \<in> \<Q> \<A> \<Longrightarrow> FP q \<longleftrightarrow> (q2_\<alpha> q) \<in> \<F> \<A>"
    and wf_\<A>: "NFA \<A>"
    and sig\<Sigma>: "\<Sigma> \<A> = semI SigI"
  shows "nfa_invar_NFA 
...
proof (intro nfa_construct_prod.intro nfa_by_map_correct 
  ...
  assume wf_\<A>: "NFA \<A>" 
     ...
     and finite_D: "finite D"
     and sig\<Sigma>: "\<Sigma> \<A> = semI Sig"
...
assumes const_OK: "nfa_construct_no_enc \<alpha>3 invar3 const"
    ...
    and Sig12_sinter: "\<And> n1 n2. invar1 n1 \<and> invar2 n2 \<Longrightarrow>
                           semI (s_inter (Sig1 n1) (Sig2 n2)) = (\<Sigma> (\<alpha>1 n1))"
...
qed qed
  ...
  have "semI (s_inter (Sig1 n1) (Sig2 n2)) = \<Sigma> (\<alpha>1 n1)" 
      by auto
  from this
  have semI\<Sigma>: "\<Sigma> (NFA_concatenation (\<alpha>1 n1) (\<alpha>2 n2)) = 
      semI (s_inter (Sig1 n1) ( Sig2 n2))"
    unfolding NFA_concatenation_def
    by simp
...
assumes const_OK: "nfa_construct_no_enc_prod \<alpha>3 invar3 const"
    ...
    and sig12_inter: "\<And> n1 n2. invar1 n1 \<and> invar2 n2 \<Longrightarrow>
                           semI (s_inter (Sig1 n1) (Sig2 n2)) = (\<Sigma> (\<alpha>1 n1) \<inter> \<Sigma> (\<alpha>2 n2))"
    and I1_OK: "\<And>n1. invar1 n1 \<Longrightarrow> distinct (I1 n1) \<and> set (I1 n1) = \<I> (\<alpha>1 n1)" 
    and I2_OK: "\<And>n2. invar2 n2 \<Longrightarrow> distinct (I2 n2) \<and> set (I2 n2) = \<I> (\<alpha>2 n2)"
...
proof (intro nfa_bool_comb.intro 
 ...
 from this it_2_OK have it_2_OK' :"
  ...
  have sig12_inter': "\<Sigma> (bool_comb_NFA bc (\<alpha>1 n1) (\<alpha>2 n2)) = semI (s_inter (Sig1 n1) (Sig2 n2))"
     ...
     thm sem_OK
      ...
      show ?thesis 
        ...
        by (simp_all add: 
              ...
              nfa.nfa_invar_def sem_OK inter_correct)
...
assumes qm_ops_OK: "StdMap qm_ops"
    ...
    and \<Sigma>eq:  "\<And>n.
       ...
       semI (nfa.nfa_alphabet n) = \<Sigma> (nfa.nfa_\<alpha> n)"
    and \<Delta>_\<A>1: "\<And> n1. nfa.nfa_invar_NFA n1 \<Longrightarrow> 
          \<exists>D1. {(q, semI a, q')| q a q'. (q, a, q') \<in> D1} = \<Delta> (nfa.nfa_\<alpha> n1) \<and>
               finite D1"
    and \<Delta>_\<A>2: "\<And> n2. nfa.nfa_invar_NFA n2 \<Longrightarrow> 
...
proof (intro nfa_concat_same.intro 
  ...
  from this
  have semIeq: "semI (nfa.nfa_alphabet n1) = semI (nfa.nfa_alphabet n2)"
  ...
  proof -
    ...
    proof -
      ...
      show ?thesis 
        ...
        defer defer
        apply (insert \<Delta>_\<A>1 \<Delta>_\<A>2 \<Delta>_it_ok1 \<Delta>_it_ok2 \<Delta>_it_ok3 Q1Q2_empty semIeq)  
        apply auto
        apply (simp_all add: 
              nfa.nfa_trans_def 
              set_iterator_def nfa.nfa_\<alpha>_def
              ...
              nfa.nfa_invar_def )
           defer
           apply (rename_tac q a af bc aa ab b ac ag bb ad ae ba D1)
           defer
           apply (rename_tac q a af bc aa ab b ac ag bb ad ae ba D1 D2)
        defer defer
        apply (rename_tac a af bc aa ab b ac ag bb ad ae ba q)
      proof -
        {
          fix a aa ab b x
       ...
       s.invar ab \<and>
       s.invar b \<and>
       NFA \<lparr>\<Q> = s.\<alpha> a, \<Sigma> = semI (af, bc), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> aa, \<I> = s.\<alpha> ab, \<F> = s.\<alpha> b\<rparr>" and
        p5: "s.invar ac \<and>
       d_nfa.invar ad \<and>
       s.invar ae \<and>
       s.invar ba \<and>
       NFA \<lparr>\<Q> = s.\<alpha> ac, \<Sigma> = semI (ag, bb), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> ad, \<I> = s.\<alpha> ae,
              \<F> = s.\<alpha> ba\<rparr>" and
           p1: "(\<And>D1 a aa b ab ac ba ad ae bb af ag bc q.
           ...
           s.invar ac \<and>
           s.invar ba \<and>
           NFA \<lparr>\<Q> = s.\<alpha> a, \<Sigma> = semI (aa, b), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> ab,
                  \<I> = s.\<alpha> ac, \<F> = s.\<alpha> ba\<rparr> \<Longrightarrow>
           s.invar ad \<and>
           d_nfa.invar af \<and>
           s.invar ag \<and>
           s.invar bc \<and>
           NFA \<lparr>\<Q> = s.\<alpha> ad, \<Sigma> = semI (ae, bb), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> af,
                  \<I> = s.\<alpha> ag, \<F> = s.\<alpha> bc\<rparr> \<Longrightarrow>
           set_iterator_genord (it_3_nfa ab ba ag q)
            {(a, q'). \<exists>q''. (q, a, q'') \<in> D1 \<and> q'' \<in> s.\<alpha> ba \<and> q' \<in> s.\<alpha> ag}
            (\<lambda>_ _. True))"   
      ...
      { fix q a aa ab b ac ad ae ba D1 D2 af bc ag bb
        assume pc1: " s.invar a \<and>
           d_nfa.invar aa \<and>
           s.invar ab \<and>
           s.invar b \<and>
           NFA \<lparr>\<Q> = s.\<alpha> a, \<Sigma> = semI (af, bc), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> aa, \<I> = s.\<alpha> ab,
       ...
       nfa.interval_to_set ` d_nfa.\<alpha> ad" and
        ...
        cp1: "\<A>2 =  \<lparr>\<Q> = s.\<alpha> ac, \<Sigma> = semI (ag, bb), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> ad, \<I> = s.\<alpha> ae,
                  \<F> = s.\<alpha> ba\<rparr>" and
        cp2: "\<A>1 = \<lparr>\<Q> = s.\<alpha> a, \<Sigma> = semI (af, bc), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> aa, \<I> = s.\<alpha> ab,
                  \<F> = s.\<alpha> b\<rparr>" and
        cp3: "NFA \<A>1" and
        cp4: "NFA \<A>2"
          by auto
        from cp1 obtain n1' n2' where
        n2'_def: "n2' = (ac, (ag, bb), ad, ae, ba)" and
        n1'_def: "n1' = (a, (af, bc), aa, ab, b)"
          by auto
      ...
      {
        fix a aa ab b ac ad ae ba q af bc ag bb
        assume p1: "s.invar a \<and>
                    d_nfa.invar aa \<and>
                    s.invar ab \<and>
                    s.invar b \<and>
                    NFA \<lparr>\<Q> = s.\<alpha> a, \<Sigma> = semI (af, bc), \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> aa, \<I> = s.\<alpha> ab, \<F> = s.\<alpha> b\<rparr>"
       ...
       NFA \<lparr>\<Q> = s.\<alpha> ac, \<Sigma> = semI (ag, bb),  \<Delta> = nfa.interval_to_set ` d_nfa.\<alpha> ad, \<I> = s.\<alpha> ae,
              \<F> = s.\<alpha> ba\<rparr>"
          and p3: "(\<And>a aa b ab ac ba ad ae bb af ag bc.
           s.invar a \<and>
           d_nfa.invar ab \<and>
           s.invar ac \<and>
           s.invar ba \<and>
...
fun nfa_concat_rename_impl where
    "nfa_concat_rename_impl qm_ops it_1_nfa it_2_nfa it_3_nfa
      im1 im2 f1 f2 
        (A1::'q_set \<times> ('a \<times> 'a) \<times> 'd_nfa \<times> 'q_set \<times> 'q_set) A2 =
        (concat_rename_impl_aux 
        (ss.inter)
...
definition nfa_\<alpha>p :: "'qq_set \<times> ('a \<times> 'a) \<times> 'dd_nfa \<times> 'qq_set \<times> 'qq_set \<Rightarrow> ('q\<times>'q, 'a) NFA_rec" 
  where
  "nfa_\<alpha>p A =
   \<lparr> \<Q> = ss.\<alpha> (nfa.nfa_statep A), 
     \<Sigma> = semI (nfa.nfa_alphap A),
     \<Delta> = interval_to_set ` (dd_nfa.\<alpha> (nfa.nfa_transp A)),
     \<I> = ss.\<alpha> (nfa.nfa_initialp A), 
     \<F> = ss.\<alpha> (nfa.nfa_acceptingp A) \<rparr>"

definition nfa_invarp :: "'qq_set \<times> ('a \<times> 'a) \<times> 'dd_nfa \<times> 'qq_set \<times> 'qq_set \<Rightarrow> bool" where
  "nfa_invarp A =
...
proof (intro nfa_concat_rename_same.intro 
        ...
        }
  {
        ...
        (it_3_nfa (fst (snd (snd n1a))) (nfa.nfa_acceptingp n1a) (nfa.nfa_initialp n2a) q)
        {uu.
         \<exists>a b aa ba ab bb.
            uu = ((a, b), ab, bb) \<and>
            (q, (a, b), aa, ba) \<in> D1 \<and>
