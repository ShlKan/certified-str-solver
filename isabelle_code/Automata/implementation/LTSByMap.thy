...
fun ltsbm_union_aux1 :: "'V \<Rightarrow> 'W \<Rightarrow> 'V list \<Rightarrow> 'm1 \<Rightarrow> 'm1" where
  "ltsbm_union_aux1 v w [] m1 = m1" |
  "ltsbm_union_aux1 v w (a # l) m1 = ltsbm_union_aux1 v w l (ltsbm_add v w a m1)"


lemma union_aux1_alt: 
    "ltsbm_union_aux1 v w l m1 = List.foldl (\<lambda> s q. ltsbm_add v w q s) m1 l"
  apply (induction l arbitrary: m1)
  apply simp
  by simp

lemma ltsbm_union_aux1_correct:
  assumes invarm1: "ltsbm_invar m1"
  shows "ltsbm_invar (ltsbm_union_aux1 v w vs m1)"
        "ltsbm_\<alpha> (ltsbm_union_aux1 v w vs m1) = 
              {(v, w, v') | v'. v' \<in> set vs} \<union> (ltsbm_\<alpha> m1)"
   using invarm1
   apply (induction vs arbitrary: m1)
   using invarm1 apply force
   using ltsbm_add_correct
         lts_add_def[of ltsbm_\<alpha> ltsbm_invar ltsbm_add]
   apply force 
   apply (metis (no_types, lifting) lts_add.lts_add_correct(1) 
              ltsbm_add_correct ltsbm_union_aux1.simps(2)) 
 proof -
   {
     fix a vs m1
     assume semind: "(\<And>m1. ltsbm_invar m1 \<Longrightarrow>
              ltsbm_\<alpha> (ltsbm_union_aux1 v w vs m1) =
              {(v, w, v') |v'. v' \<in> set vs} \<union> ltsbm_\<alpha> m1)"
        and invarm1: "ltsbm_invar m1"
     from invarm1 ltsbm_add_correct
     have invaradd:"ltsbm_invar (ltsbm_add v w a m1)"
       unfolding lts_add_def
       by force

     from ltsbm_add_correct
     have ltsaddsem: "ltsbm_\<alpha> (ltsbm_add v w a m1) = {(v, w, a)} \<union> (ltsbm_\<alpha> m1)"
       unfolding lts_add_def
       using invarm1 by blast

     from semind[OF invaradd] ltsaddsem
     show "ltsbm_\<alpha> (ltsbm_union_aux1 v w (a # vs) m1) =
              {(v, w, v') |v'. v' \<in> set (a # vs)} \<union> ltsbm_\<alpha> m1"
       unfolding ltsbm_union_aux1.simps
       by simp
   }
 qed

fun ltsbm_union_aux2 :: "'V \<Rightarrow> ('W \<times> 's3) list \<Rightarrow> 'm1 \<Rightarrow> 'm1" where
  "ltsbm_union_aux2 v [] m1 = m1" |
  "ltsbm_union_aux2 v (t # l) m1 = ltsbm_union_aux2 v l 
                                (ltsbm_union_aux1 v (fst t) (s3.to_list (snd t)) m1)"

lemma union_aux2_alt: 
    "ltsbm_union_aux2 v l m1 = 
        List.foldl (\<lambda> s t. ltsbm_union_aux1 v (fst t) (s3.to_list (snd t)) s) m1 l"
  apply (induction l arbitrary: m1)
  apply simp
  by simp

lemma ltsbm_union_aux2_correct:
  assumes invarm1: "ltsbm_invar m1"
     and invar2: "\<forall> t \<in> set ts. s3.invar (snd t)"
  shows "ltsbm_invar (ltsbm_union_aux2 v ts m1)"
        "ltsbm_\<alpha> (ltsbm_union_aux2 v ts m1) = 
              {(v, w, v') | w v' t. t \<in> set ts \<and> w = fst t \<and> 
                            v' \<in> set (s3.to_list (snd t))} 
                 \<union> (ltsbm_\<alpha> m1)"
  using invarm1 invar2
  apply (induction ts arbitrary: m1)
  apply simp  
  apply simp
  apply (simp add: ltsbm_union_aux1_correct(1))
proof -
  fix a ts m1
  assume semind: "(\<And>m1. ltsbm_invar m1 \<Longrightarrow> \<forall>t\<in>set ts. s3.invar (snd t) \<Longrightarrow>
              ltsbm_\<alpha> (ltsbm_union_aux2 v ts m1) =
              {uu.
               \<exists>w v' t.
                  uu = (v, w, v') \<and>
                  t \<in> set ts \<and> w = fst t \<and> v' \<in> set (s3.to_list (snd t))} \<union>
              ltsbm_\<alpha> m1)"
     and invarm1: "ltsbm_invar m1"
     and s3invar: "\<forall>t\<in>set (a # ts). s3.invar (snd t)"

  from s3invar
  have s3invar': "\<forall>t\<in>set ts. s3.invar (snd t)"
    by force

  from  ltsbm_union_aux1_correct(1)[OF invarm1, of v "fst a" 
                                               "(s3.to_list (snd a))"]
  have invarm1union: "ltsbm_invar (ltsbm_union_aux1 v (fst a) (s3.to_list (snd a)) m1)"
    by simp

  from ltsbm_union_aux1_correct(2)[OF invarm1, of v "fst a" 
                                               "(s3.to_list (snd a))"]
  have semu: "ltsbm_\<alpha> (ltsbm_union_aux1 v (fst a) (s3.to_list (snd a)) m1) =
             {(v, fst a, v') |v'. v' \<in> set (s3.to_list (snd a))} \<union> ltsbm_\<alpha> m1"
    by simp
  
  thm s3invar s3invar' 
  have seteq: "{(v, w, v') |w v'. \<exists>b. (w, b) \<in> set ts \<and> v' \<in> set (s3.to_list b)} \<union>
          {(v, fst a, v') |v'. v' \<in> set (s3.to_list (snd a))} = 
        {(v, w, v') |w v'. \<exists>b. ((w, b) = a \<or> (w, b) \<in> set ts) 
                \<and> v' \<in> set (s3.to_list b)}" 
    apply (rule)
    apply (force)
    by (force)
  from semind[OF invarm1union s3invar'] semu
  show "ltsbm_\<alpha> (ltsbm_union_aux2 v (a # ts) m1) =
       {uu.
        \<exists>w v' t.
           uu = (v, w, v') \<and>
           t \<in> set (a # ts) \<and> w = fst t \<and> v' \<in> set (s3.to_list (snd t))} \<union>
       ltsbm_\<alpha> m1"
    unfolding ltsbm_union_aux2.simps
    apply simp
    using seteq 
    by (metis (no_types, lifting) Un_assoc)
qed

lemma mapof: "\<forall> w v l. (w, v) \<in>set l \<and> distinct (map fst l) \<longrightarrow> map_of l w = Some v"
    apply (rule_tac allI impI)+
  proof -
    fix w v l
    assume p1: "(w, v) \<in> set l \<and> distinct (map fst l)"
    from this
    show "map_of l w = Some v"
      apply (induction l)
       apply simp
    proof -
      fix a l
      assume indhyp: "((w, v) \<in> set l \<and> distinct (map fst l) \<Longrightarrow> map_of l w = Some v)"
         and pre: "(w, v) \<in> set (a # l) \<and> distinct (map fst (a # l))"
      show "map_of (a # l) w = Some v"
      proof (cases "(w,v) = a")
        case True
        from this map_of_Cons_code(2)[of w v l w]
        show ?thesis by auto
      next
        case False
        from this pre
        have "(w, v) \<in> set l \<and> distinct (map fst l)" by auto
        from this indhyp map_of_Cons_code(2)[of w v l w]
        show ?thesis 
          using pre by auto
      qed
    qed
...
lemma mapof': "\<forall>w v l. map_of l w = Some v \<and> distinct (map fst l) \<longrightarrow> (w, v) \<in> set l"
    apply (rule allI impI) +
  proof -
    fix w v l
    assume p1: "map_of l w = Some v \<and> distinct (map fst l)"
    from this
    show "(w, v) \<in> set l"
      apply (induction l)
       apply simp
    proof -
      fix a l
      assume p2: "(map_of l w = Some v \<and> distinct (map fst l) \<Longrightarrow> (w, v) \<in> set l)"
         and p3: "map_of (a # l) w = Some v \<and> distinct (map fst (a # l))"
      show "(w, v) \<in> set (a # l)"
      proof (cases "(w, v) = a")
      case True
      ...
      next
        case False
        from this p3 
        have c1: "map_of l w = Some v \<and> distinct (map fst l)"
          by (metis distinct.simps(2) list.simps(9) 
                map_of_Cons_code(2) option.inject prod.collapse)
        from p2[OF c1] 
        show ?thesis by auto
      qed
    qed
...
lemma ltsbm_union_aux2_correct':
  assumes invarm1: "ltsbm_invar m1 \<and> m1.\<alpha> m1 v = Some m2"
     and invar2: "ltsbm_invar m1'"
  shows "ltsbm_invar (ltsbm_union_aux2 v (m2.to_list m2) m1')"
        "ltsbm_\<alpha> (ltsbm_union_aux2 v (m2.to_list m2) m1') = 
              {(v, w, v') | w v' s. m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s} 
                 \<union> (ltsbm_\<alpha> m1')"

proof -
  from invarm1
  have m2dom: "m2\<in>ran (m1.\<alpha> m1) \<and> m2.invar m2"
    unfolding ltsbm_invar_def 
    by (meson ranI)
  
  
     
  from mapof m2.correct(26)[of m2] m2dom invarm1 m2.correct(25)[of m2]
  have pre2: "\<forall>t\<in>set (m2.to_list m2). s3.invar (snd t)"
    unfolding ltsbm_invar_def 
    using invarm1 ltsbm_invar_alt_def 
    by fastforce
    
    
  from ltsbm_union_aux2_correct[of m1' "(m2.to_list m2)" v, OF invar2 pre2]
  show "ltsbm_invar (ltsbm_union_aux2 v (m2.to_list m2) m1')"
    by simp

  

  have seteq: "{uu. \<exists>w v' s. uu = (v, w, v') \<and> m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s} = 
              {uu. \<exists>w v' t.  uu = (v, w, v') \<and> t \<in> set (m2.to_list m2) \<and> w = fst t 
                \<and> v' \<in> set (s3.to_list (snd t))}"
  proof 
    {
      show "{uu. \<exists>w v' s. uu = (v, w, v') \<and> m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s}
            \<subseteq> {uu. \<exists>w v' t.
               uu = (v, w, v') \<and>
               t \<in> set (m2.to_list m2) \<and> w = fst t \<and> v' \<in> set (s3.to_list (snd t))}"
      proof
        fix x
        assume xin: "x \<in> {uu. \<exists>w v' s. uu = (v, w, v') \<and> 
                            m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s}"
        from this obtain w v' s  where
        wv'_def: "x = (v, w, v') \<and> m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s" by auto
        from this m2.correct(26)[of m2] m2.correct(25)[OF] m2dom  mapof' 
        have c1: "(w, s) \<in> set (m2.to_list m2)" by force
        from this wv'_def s3.correct
        have c2: "w = fst (w, s) \<and> v' \<in> set (s3.to_list (snd (w, s)))"
          by (metis fst_conv pre2 snd_conv)
        from c1 c2 wv'_def
        show "x \<in> {uu. \<exists>w v' t.
                      uu = (v, w, v') \<and>
                      t \<in> set (m2.to_list m2) \<and> w = fst t \<and> 
                        v' \<in> set (s3.to_list (snd t))}"
          by force
      qed
    }
    {
     ...
     \<exists>w v' t.
        uu = (v, w, v') \<and>
        t \<in> set (m2.to_list m2) \<and> w = fst t \<and> v' \<in> set (s3.to_list (snd t))}
    \<subseteq> {uu. \<exists>w v' s. uu = (v, w, v') \<and> m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s}"
      proof 
        fix x
        assume xin: "x \<in> {uu. \<exists>w v' t.
                     uu = (v, w, v') \<and>
                     t \<in> set (m2.to_list m2) \<and>
                     w = fst t \<and> v' \<in> set (s3.to_list (snd t))}"
        from this obtain w v' t
          where wv'_def: "x = (v, w, v') \<and> (w, t) \<in> set (m2.to_list m2)
                       \<and> v' \<in> set (s3.to_list t)"
          by auto
        from this m2.correct(26)[of m2] m2.correct(25)[OF] m2dom  mapof
        have c1: "m2.\<alpha> m2 w = Some t" by force
        from this wv'_def s3.correct invarm1
        show "x \<in> {uu. \<exists>w v' s. uu = (v, w, v') \<and> m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s}"
          by (smt mem_Collect_eq pre2 snd_conv)
      qed
    }
  qed
  from this ltsbm_union_aux2_correct[of m1' "(m2.to_list m2)" v, OF invar2 pre2]
  show "ltsbm_\<alpha> (ltsbm_union_aux2 v (m2.to_list m2) m1') =
             {uu. \<exists>w v' s. uu = (v, w, v') \<and> m2.\<alpha> m2 w = Some s \<and> v' \<in> s3.\<alpha> s} 
               \<union> ltsbm_\<alpha> m1'"
    by auto
qed

fun ltsbm_union_aux3 :: "('V \<times> 'm2) list \<Rightarrow> 'm1 \<Rightarrow> 'm1" where
   "ltsbm_union_aux3 [] m1 = m1" | 
   "ltsbm_union_aux3 (t # l) m1 = ltsbm_union_aux3 l 
                (ltsbm_union_aux2 (fst t) (m2.to_list (snd t)) m1)"

lemma union_aux3_alt: 
    "ltsbm_union_aux3 l m1 = 
        List.foldl (\<lambda> s t. ltsbm_union_aux2 (fst t) (m2.to_list (snd t)) s) m1 l"
  apply (induction l arbitrary: m1)
  apply simp
  by simp

lemma to_list_empy: "m1.to_list m1 = [] \<and> m1.invar m1 \<longrightarrow> 
                      (\<forall> e. m1.\<alpha> m1 e = None)" 
proof 
  assume p1: "m1.to_list m1 = [] \<and> m1.invar m1"
  from this m1.correct(26)[of m1] m1.correct(25)[of m1] mapof'
  show "\<forall>e. m1.\<alpha> m1 e = None"
    by (metis m1.to_list_correct(1) map_of_Cons_code(1))
qed

thm ltsbm_union_aux2_correct

lemma ltsbm_union_aux3_correct:
  assumes invarm1': "ltsbm_invar m1'"
       and invarl : "\<forall> (v, t) \<in> set l. m2.invar t \<and> (\<forall> s3 \<in> ran (m2.\<alpha> t). s3.invar s3)"
  shows "ltsbm_invar (ltsbm_union_aux3 l m1')"
        "ltsbm_\<alpha> (ltsbm_union_aux3 l m1') = 
              {(v,w,v'). \<exists> m2 s3. (v, m2) \<in> set l \<and> 
                                  m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} 
                 \<union> (ltsbm_\<alpha> m1')"
  using invarm1' invarl
  apply (induction l arbitrary: m1')
  using invarm1' apply force
  apply force 
proof -
  {
    fix a l m1'
    assume ind_hyp: "(\<And>m1'. ltsbm_invar m1' \<Longrightarrow> 
                          \<forall>(v, t)\<in>set l. m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar \<Longrightarrow>
                          ltsbm_invar (ltsbm_union_aux3 l m1'))"
           and p1: "ltsbm_invar m1'" 
           and p2: "\<forall>(v, t)\<in>set (a # l). m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar"

    from p2 have c3: "\<forall>(v, t)\<in>set l. m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar" by auto

    from p1
    have m1invar: "m1.invar m1'"
      unfolding ltsbm_invar_def
      by simp

    from p2 have m2invar: "m2.invar (snd a)" by auto

    from p2 have m2invar': "Ball (ran (m2.\<alpha> (snd a))) s3.invar" by auto
   
    from this p2 m2.correct(25)[OF m2invar] 
         m2.correct(26)[OF m2invar] mapof m2invar'
    have "\<forall>w v. (w, v) \<in> set (m2.to_list (snd a)) \<longrightarrow>
             map_of (m2.to_list (snd a)) w = Some v"
      by fastforce
    from this m2invar' p2 m2.correct(25)[OF m2invar] 
         m2.correct(26)[OF m2invar] mapof m2invar'
    have c1: "\<forall>t\<in>set (m2.to_list (snd a)). s3.invar (snd t)"
      using ranI by fastforce

    from ltsbm_union_aux2_correct(1)[OF p1 c1]
    have c2: "ltsbm_invar (ltsbm_union_aux2 (fst a) (m2.to_list (snd a)) m1')"
      by simp
    
    from ind_hyp[OF c2 c3] 
    show "ltsbm_invar (ltsbm_union_aux3 (a # l) m1')"
      unfolding ltsbm_union_aux3.simps by simp
  }

  {
    ...
    assume ind_hyp: "(\<And>m1'. ltsbm_invar m1' \<Longrightarrow>
               ...
               ltsbm_\<alpha> (ltsbm_union_aux3 l m1') =
               {(v, w, v').
                \<exists>m2 s3. (v, m2) \<in> set l \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} \<union>
               ltsbm_\<alpha> m1')"
       and p1: "ltsbm_invar m1'"
       and p2: "\<forall>(v, t)\<in>set (a # l). m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar"
    thm ltsbm_union_aux2_correct(2)[OF p1, of "m2.to_list (snd a)" "fst a"]
    ...
    from this m2invar' p2 m2.correct(25)[OF m2invar] 
         m2.correct(26)[OF m2invar] mapof 
    have c0: "\<forall>t\<in>set (m2.to_list (snd a)). s3.invar (snd t)"
      using ranI by fastforce

    from ltsbm_union_aux2_correct(1)[of m1' "(m2.to_list (snd a))" "fst a", OF p1 c0]
    have c1: "ltsbm_invar (ltsbm_union_aux2 (fst a) (m2.to_list (snd a)) m1')"
      by simp

    from p2
    have c4: "\<forall>(v, t)\<in>set l. m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar"
      by simp

    have seteq1: "{(v, w, v'). \<exists>m2 s3. (v, m2) \<in> set (a # l) \<and> 
                        m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} =
          {(v, w, v'). \<exists>m2. (v, m2) \<in> set l \<and> 
                       (\<exists>s3. m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3)} \<union> 
          {((fst a), w, v')| w v'. \<exists> s3. m2.\<alpha> (snd a) w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
      apply (rule)
      apply force
      apply (rule)
    proof -
      fix x
      assume p1: " x \<in> {(v, w, v').
              \<exists>m2. (v, m2) \<in> set l \<and> (\<exists>s3. m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3)} \<union>
             {(fst a, w, v') |w v'. \<exists>s3. m2.\<alpha> (snd a) w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
      show "x \<in> {(v, w, v').
               \<exists>m2 s3. (v, m2) \<in> set (a # l) \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
      proof (cases "x \<in> {(fst a, w, v') |w v'. 
              \<exists>s3. m2.\<alpha> (snd a) w = Some s3 \<and> v' \<in> s3.\<alpha> s3}")
        case True 
        from p1 True
        obtain b q' where "x = (fst a, b , q')" by auto
        from this True obtain s3
          where s3_def: "m2.\<alpha> (snd a) b = Some s3 \<and> q' \<in> s3.\<alpha> s3"
          by auto
        from this True
        have "(fst a, (snd a)) \<in> set (a # l) \<and> m2.\<alpha> (snd a) b = Some s3 \<and> 
               q' \<in> s3.\<alpha> s3" by force
        from this
        show ?thesis
          using \<open>x = (fst a, b, q')\<close> by blast
      next
        case False
        from this p1 have "x \<in> {(v, w, v').
              \<exists>m2. (v, m2) \<in> set l \<and> (\<exists>s3. m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3)}" by auto
        then show ?thesis 
          by auto
      qed
    qed
    have seteq2: "{(fst a, w, v') |w v'.
        \<exists>b. (w, b) \<in> set (m2.to_list (snd a)) \<and> v' \<in> set (s3.to_list b)} = 
      {(fst a, w, v') |w v'. \<exists>s3. m2.\<alpha> (snd a) w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
      apply (rule)
      apply (rule)
      defer 
    ...
    proof -
      {
        fix x
        assume p1: "x \<in> {(fst a, w, v') |w v'. \<exists>s3. m2.\<alpha> (snd a) 
                      w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
        from p2
        have m2invar: "m2.invar (snd a)" by auto
        from p1 obtain w v' s3
         where wv'_def: "x = (fst a, w ,v') \<and> m2.\<alpha> (snd a) 
                      w = Some s3 \<and> v' \<in> s3.\<alpha> s3" by auto
        from this m2.correct(25)[of "snd a", OF m2invar] mapof
             m2.correct(26)[OF m2invar] s3.correct
        have "(w, s3) \<in> set (m2.to_list (snd a)) \<and> v' \<in> set (s3.to_list (s3))"
          by (metis c0 map_of_SomeD snd_conv)
        from this wv'_def
        show "x \<in> {(fst a, w, v') |w v'.
               \<exists>b. (w, b) \<in> set (m2.to_list (snd a)) \<and> v' \<in> set (s3.to_list b)}"
          by auto
      }
      {
        fix x
        assume p1: "x \<in> {(fst a, w, v') |w v'.
              \<exists>b. (w, b) \<in> set (m2.to_list (snd a)) \<and> v' \<in> set (s3.to_list b)}"
        from this 
        obtain w v' b where
        wv'_def: "x = (fst a, w, v') \<and> 
              (w, b) \<in> set (m2.to_list (snd a)) \<and> v' \<in> set (s3.to_list b)" by auto
        from this m2.correct(25)[of "snd a", OF m2invar] mapof
             m2.correct(26)[OF m2invar] s3.correct
        have "m2.\<alpha> (snd a) w = Some b \<and> v' \<in> s3.\<alpha> b"
          by (metis c0 map_of_SomeD snd_conv)
        from this wv'_def
        show "x \<in> {(fst a, w, v') |w v'. \<exists>s3. m2.\<alpha> (snd a) w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
          by auto
      }
    qed

    from seteq1 seteq2
    have seteq: "{(v, w, v'). \<exists>m2. (v, m2) \<in> set l \<and> (\<exists>s3. m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3)} \<union>
          {(fst a, w, v') |w v'. \<exists>b. (w, b) \<in> set (m2.to_list (snd a)) \<and> v' \<in> set (s3.to_list b)}
          = {(v, w, v'). \<exists>m2 s3. (v, m2) \<in> set (a # l) \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
      by auto

    have seteq3: "{(v, w, v').
     \<exists>m2. ((v, m2) = a \<or> (v, m2) \<in> set l) \<and> (\<exists>s3. m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3)} = 
     {(v, w, v'). \<exists>m2. (v, m2) \<in> set l \<and> (\<exists>s3. m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3)} \<union>
     {(fst a, w, v') |w v'. \<exists>s3. m2.\<alpha> (snd a) w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
      apply (rule)
      apply force
      apply force
    ...
    show "ltsbm_\<alpha> (ltsbm_union_aux3 (a # l) m1') = {(v, w, v'). \<exists>m2 s3. (v, m2) \<in> set (a # l) \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} \<union>
       ltsbm_\<alpha> m1'"
      unfolding ltsbm_union_aux3.simps 
      apply (simp add: ind_hyp[of "(ltsbm_union_aux2 (fst a) (m2.to_list (snd a)) m1')",
                OF c1 c4]
         ltsbm_union_aux2_correct(2)[of m1' "m2.to_list (snd a)" "fst a", OF p1 c0])
      using seteq seteq3
      by (simp add: Un_assoc seteq2)
  }
qed
...
lemma ltsbm_union_aux3_correct':
  assumes invarm1': "ltsbm_invar m1'"
      and invarm1 : "ltsbm_invar m1"
  shows "ltsbm_invar (ltsbm_union_aux3 (m1.to_list m1) m1')"
        "ltsbm_\<alpha> (ltsbm_union_aux3 (m1.to_list m1) m1') = 
              {(v,w,v'). \<exists> m2 s3. m1.\<alpha> m1 v = Some m2 \<and> 
                                  m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} 
                 \<union> (ltsbm_\<alpha> m1')"
proof -

  from invarm1
  have m1invar: "m1.invar m1" unfolding ltsbm_invar_def by auto

  from invarm1 mapof m1.correct(25)[OF m1invar]
       m1.correct(26)[OF m1invar]
  have c0: "\<forall>(v, t)\<in>set (m1.to_list m1). m1.\<alpha> m1 v = Some t"
    by fastforce
  from this invarm1
  have c1: "\<forall> v t. 
      (v, t)\<in>set (m1.to_list m1) \<longrightarrow> m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar"
    unfolding ltsbm_invar_def
    apply (rule_tac allI impI)+
  proof -
    fix v t
    assume p1: "(v, t) \<in> set (m1.to_list m1)"
    from p1 c0
    have c2: "m1.\<alpha> m1 v = Some t" by auto
    from this invarm1
    show "m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar"
      unfolding ltsbm_invar_def
      by (meson ranI)
  qed
  from this
  have c3: "\<forall>(v, t)\<in>set (m1.to_list m1). m2.invar t \<and> Ball (ran (m2.\<alpha> t)) s3.invar "
    by auto

  note aux3 = ltsbm_union_aux3_correct[OF invarm1', of "m1.to_list m1",OF c3]
  from aux3
  show "ltsbm_invar (ltsbm_union_aux3 (m1.to_list m1) m1')"
    by simp

  from invarm1 mapof' m1.correct(25)[OF m1invar]
       m1.correct(26)[OF m1invar]
  have c00: "\<forall>v t. m1.\<alpha> m1 v = Some t \<longrightarrow> (v, t)\<in>set (m1.to_list m1)"
    by force

  have seteq: "{(v, w, v').
     \<exists>m2 s3. (v, m2) \<in> set (m1.to_list m1) \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} = 
    {(v, w, v'). \<exists>m2 s3. m1.\<alpha> m1 v = Some m2 \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3}"
    apply (rule)
    using c0 apply force
    using c00 apply force
    done

  from aux3(2) c0 seteq
  show "ltsbm_\<alpha> (ltsbm_union_aux3 (m1.to_list m1) m1') =
    {(v, w, v'). \<exists>m2 s3. m1.\<alpha> m1 v = Some m2 \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} \<union>
    ltsbm_\<alpha> m1'"
    by force
qed


definition ltsbm_union :: "'m1 lts_union" where 
    "ltsbm_union l1 l2 \<equiv> ltsbm_union_aux3 (m1.to_list l1) l2"

lemma ltsbm_union_correct':
  assumes invarm1': "ltsbm_invar m1'"
      and invarm1 : "ltsbm_invar m1"
  shows "ltsbm_invar (ltsbm_union m1 m1')"
        "ltsbm_\<alpha> (ltsbm_union m1 m1') = 
              {(v,w,v'). 
    \<exists> m2 s3. m1.\<alpha> m1 v = Some m2 \<and> m2.\<alpha> m2 w = Some s3 \<and> v' \<in> s3.\<alpha> s3} \<union> ltsbm_\<alpha> m1'"
  unfolding ltsbm_union_def
proof -
  note aux = ltsbm_union_aux3_correct'[OF invarm1' invarm1]
  from aux
  show "ltsbm_invar (ltsbm_union_aux3 (m1.to_list m1) m1')"
    by simp
...
lemma ltsbm_union_correct_aux:
  assumes invarm1': "ltsbm_invar m1'"
      and invarm1 : "ltsbm_invar m1"
  shows "ltsbm_invar (ltsbm_union m1 m1')"
        "ltsbm_\<alpha> (ltsbm_union m1 m1') = 
              ltsbm_\<alpha> m1 \<union> ltsbm_\<alpha> m1'"
  using ltsbm_union_correct'
  using invarm1 invarm1' apply blast
  using ltsbm_union_correct'[OF invarm1' invarm1] ltsbm_\<alpha>_def[of m1] 
  by auto

lemma ltsbm_union_correct:
    "lts_union ltsbm_\<alpha> ltsbm_invar ltsbm_union"
  unfolding lts_union_def
  using ltsbm_union_correct_aux
  by force
...
definition ltsbm_succq_it where
    "ltsbm_succq_it it m1 v e =
     (case m1.lookup v m1 of
         None \<Rightarrow> set_iterator_emp
       | Some m2 \<Rightarrow>
           (case m2.lookup e m2 of
               None \<Rightarrow> set_iterator_emp
             | Some s3 \<Rightarrow> (\<lambda>c f \<sigma>0. it s3 c f \<sigma>0)))"
