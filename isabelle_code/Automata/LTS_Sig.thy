...
lemma LTS_is_reachable_concat':
  fixes q q' q'' \<pi> \<pi>' Trans
  assumes fst_part: "LTS_is_reachable Trans q \<pi> q'"
      and snd_part: "LTS_is_reachable Trans q' \<pi>' q''"
    shows "LTS_is_reachable Trans q (\<pi> @ \<pi>') q''"
  ...
  qed  
   ...
   next
     ...
     have "snd (last \<pi>1) = last l1"
       by (metis (no_types, opaque_lifting) 
        False LTS_path_def \<pi>\<pi>1\<pi>2 append_butlast_last_id last.simps last_appendR list.distinct(1) list.map(2) map_append map_is_Nil_conv)
...
(* Often, one is interested in deterministic transition
 systems. A transition system $\Delta$ is deterministic iff given a
 start state $q$ and a label $\sigma$ there is at most one successor
 state $q'$ such that $(q, \sigma, q') \in \Delta$ holds. In the following
 $\delta$ is used to denote deterministic transition systems. 
 Therefore, these transition systems are functions.*)

definition LTS_is_weak_deterministic :: "('q, 'a) LTS \<Rightarrow> bool" where
  "LTS_is_weak_deterministic \<Delta> = 
     ((\<forall>q \<sigma> q'. (q, \<sigma>, q') \<in> \<Delta> \<longrightarrow> \<sigma> \<noteq> {}) \<and>
...
lemma DLTS_to_LTS___LTS_is_weak_deterministic [simp] :
  assumes "\<forall> q \<sigma> q' \<sigma>'. (\<delta> (q,\<sigma>) \<noteq> None \<and> \<delta> (q',\<sigma>')\<noteq> None) \<longrightarrow> 
              \<sigma> \<noteq> {} \<and> \<sigma>' \<noteq> {} \<and> \<sigma> \<inter> \<sigma>' = {}" 
  shows "LTS_is_weak_deterministic (DLTS_to_LTS \<delta>)"
  apply (simp add: LTS_is_weak_deterministic_def)
  using assms by blast


definition LTS_to_DLTS :: "('q, 'a) LTS \<Rightarrow> ('q, 'a) DLTS" where
  "LTS_to_DLTS \<Delta> \<equiv> (\<lambda>(q, \<sigma>). if (\<exists>q'. (q,\<sigma>,q') \<in> \<Delta>) 
                              then Some (SOME q'. (q, \<sigma>, q') \<in> \<Delta>) 
                              else None)"
...
lemma LTS_to_DLTS_is_some : 
  ...
  apply (auto simp add: LTS_to_DLTS_def)
  apply (rule_tac someI_ex)
  apply auto
  by (metis (mono_tags) not_None_eq option.simps(1) someI)

lemma LTS_to_DLTS_is_none : 
  "(LTS_to_DLTS \<Delta> (q,\<sigma>) = None) \<longleftrightarrow> (\<forall>q'. (q, \<sigma>, q') \<notin> \<Delta>)"
  by (simp add: LTS_to_DLTS_def)

lemma LTS_to_DLTS_is_some_det :
  "LTS_is_weak_deterministic \<Delta>  \<Longrightarrow>
   ((LTS_to_DLTS \<Delta> (q, \<sigma>) = Some q') \<longleftrightarrow> (q, \<sigma>, q') \<in> \<Delta>)"
  unfolding LTS_to_DLTS_def LTS_is_weak_deterministic_def
  apply simp
  by (metis (no_types, lifting) inf.idem tfl_some)
...
lemma LTS_is_reachable___LTS_is_weak_deterministic :
assumes det_\<Delta>: "LTS_is_weak_deterministic \<Delta>"
shows  "\<lbrakk>LTS_is_reachable \<Delta> q w q'; LTS_is_reachable \<Delta> q w q''\<rbrakk> \<Longrightarrow> (q' = q'')"
using assms
apply (induct w arbitrary: q)
apply (auto simp add: LTS_is_weak_deterministic_def)
  by blast

subsubsection \<open> Basic Definitions for DLTSs *} \<close>

fun DLTS_path_of :: "('q,'a) DLTS \<Rightarrow> 'q \<Rightarrow> 'a set list \<Rightarrow> ('q list) option" where
  "DLTS_path_of \<delta> q [] = Some []"
| "DLTS_path_of \<delta> q (\<sigma> # w) = 
        Option.bind (\<delta> (q, \<sigma>)) (\<lambda>q'. Option.map_option (\<lambda>ql. q' # ql) 
                               (DLTS_path_of \<delta> q' w))"

lemma LTS_is_path___DLTS_path_of :
shows "LTS_is_path (DLTS_to_LTS \<delta>) q wr =
       (DLTS_path_of \<delta> q (LTS_path_L wr) = Some (LTS_path wr))"
...
next
  ...
  obtain \<sigma> q' where \<sigma>q'_eq: "\<sigma>q' = (\<sigma>, q')" by (cases \<sigma>q', blast)

  thus ?case 
  proof (cases "\<delta> (q, \<sigma>)")
    case None thus ?thesis by (simp add: \<sigma>q'_eq)
  next
    case (Some q'')
    thus ?thesis
      by (auto simp add: \<sigma>q'_eq ind_hyp)
  qed
qed


fun DLTS_reach :: "('q, 'a) DLTS \<Rightarrow> 'q \<Rightarrow> 'a set list \<Rightarrow> 'q option" where
    "DLTS_reach \<delta> q [] = Some q"
  | "DLTS_reach \<delta> q (\<sigma> # \<pi>) = Option.bind (\<delta> (q, \<sigma>)) (\<lambda>q'. DLTS_reach \<delta> q' \<pi>)"

lemma DLTS_reach_Concat [simp] : 
  "DLTS_reach \<delta> q (\<pi> @ \<pi>') = 
        Option.bind (DLTS_reach \<delta> q \<pi>) (\<lambda>q'. DLTS_reach \<delta> q' \<pi>')"
  by (induct \<pi> arbitrary: q, auto)

lemma DLTS_reach_alt_def : 
  "DLTS_reach \<delta> q \<pi> = 
       Option.map_option (\<lambda> \<pi>l. last (q # \<pi>l)) (DLTS_path_of \<delta> q \<pi>)"
...
next
  ...
  next
    case (Some q') 
    thus ?thesis 
      unfolding DLTS_reach.simps DLTS_path_of.simps
      apply simp
      by (simp add: ind_hyp map_option.compositionality o_def)
  qed
qed 

lemma LTS_is_reachable_weak_determistic :
assumes det_\<Delta>: "LTS_is_weak_deterministic \<Delta>" and
         w_\<pi> : "inPath w \<pi>"      
shows "DLTS_reach (LTS_to_DLTS \<Delta>) q \<pi> = Some q' \<Longrightarrow> 
                             LTS_is_reachable \<Delta> q w q'"
proof -
  from det_\<Delta> w_\<pi>
  show "(DLTS_reach (LTS_to_DLTS \<Delta>) q \<pi> = Some q') \<Longrightarrow> 
            LTS_is_reachable \<Delta> q w q'"
proof (induction w arbitrary: q \<pi>)
  case Nil 
  thus ?case
    by (metis DLTS_reach.simps(1) LTS_is_reachable.simps(1) inPath.elims(2) list.simps(3) option.inject)
next
  case (Cons a w) 
  note c1 = Cons(1)
  note c2 = Cons(2)
  ...
  obtain \<sigma> \<pi>' where
  \<sigma>_\<pi>'_def: "\<pi> = \<sigma> # \<pi>'"
    using c4 inPath.elims(2) by blast
  from this c2 
  obtain q'' where
  q'_def: "(q, \<sigma>, q'') \<in> \<Delta>"
    by (metis DLTS_reach.simps(2) LTS_to_DLTS_is_none bind_eq_Some_conv not_Some_eq)
  from this have "a \<in> \<sigma>"
    using \<sigma>_\<pi>'_def c4 by auto
  from this have pre1: " DLTS_reach (LTS_to_DLTS \<Delta>) q'' \<pi>' = Some q'"
    using LTS_to_DLTS_is_some_det \<sigma>_\<pi>'_def c2 det_\<Delta> q'_def by force
  have pre2: "inPath w \<pi>'"
    using \<sigma>_\<pi>'_def c4 by auto
  from c1[of q'' \<pi>', OF pre1 c3 pre2] c2 c3 c4
  show ?case
    using \<open>a \<in> \<sigma>\<close> q'_def by auto
qed qed

lemma LTS_is_reachable_weak_determistic2 :
assumes det_\<Delta>: "LTS_is_weak_deterministic \<Delta>"
shows "LTS_is_reachable \<Delta> q w q' \<Longrightarrow>
           \<exists> \<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q \<pi> = Some q'"
proof (induction w arbitrary: q, auto)
  {
    have "DLTS_reach (LTS_to_DLTS \<Delta>) q' [] = Some q'"
      by simp
    from this
    show "\<exists>\<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q' \<pi> = Some q'"
      by blast
  }
  {
    fix a w q q'' \<sigma>
    assume p1: "(\<And>q. LTS_is_reachable \<Delta> q w q' \<Longrightarrow>
             \<exists>\<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q \<pi> = Some q')"
       and p2: "a \<in> \<sigma>"
       and p3: "(q, \<sigma>, q'') \<in> \<Delta>"
       and p4: "LTS_is_reachable \<Delta> q'' w q'"
    from p1[of q'', OF p4]
    obtain \<pi> where
    \<pi>_def: "DLTS_reach (LTS_to_DLTS \<Delta>) q'' \<pi> = Some q'"
      by auto
    from this p2 p3
    have "DLTS_reach (LTS_to_DLTS \<Delta>) q (\<sigma> # \<pi>) = Some q'"
      using LTS_to_DLTS_is_some_det det_\<Delta> by force
    from this
    show "\<exists>\<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q \<pi> = Some q'"
      by blast
  }
...
lemma LTS_is_reachable_weak_determistic3 :
assumes det_\<Delta>: "LTS_is_weak_deterministic \<Delta>"
shows "LTS_is_reachable \<Delta> q w q' \<Longrightarrow>
           \<exists> \<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q \<pi> = Some q' \<and> inPath w \<pi>"
proof (induction w arbitrary: q, auto)
{
    have "DLTS_reach (LTS_to_DLTS \<Delta>) q' [] = Some q' \<and> inPath [] []"
      by simp
    from this
    show "\<exists>\<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q' \<pi> = Some q' \<and> inPath [] \<pi>"
      by blast
  }
  {
    fix a w q q'' \<sigma>
    ...
    have "DLTS_reach (LTS_to_DLTS \<Delta>) q (\<sigma> # \<pi>) = Some q'"
      using LTS_to_DLTS_is_some_det det_\<Delta> by force
    from this
    show "\<exists>\<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q \<pi> = Some q' \<and> inPath (a # w) \<pi>"
      by (metis (full_types) DLTS_reach.simps(2) LTS_to_DLTS_is_some_det \<open>\<exists>\<pi>. DLTS_reach (LTS_to_DLTS \<Delta>) q'' \<pi> = Some q' \<and> inPath w \<pi>\<close> bind.bind_lunit det_\<Delta> inPath.simps(2) p2 p3)
  }
qed


lemma LTS_is_reachable_DLTS_to_LTS [simp] :
 assumes inPath_w_\<pi>: "inPath w \<pi>"
 shows "(DLTS_reach \<delta> q \<pi> = Some q') \<Longrightarrow> 
         LTS_is_reachable (DLTS_to_LTS \<delta>) q w q'"
proof -
  from inPath_w_\<pi>
  show "(DLTS_reach \<delta> q \<pi> = Some q') \<Longrightarrow> LTS_is_reachable (DLTS_to_LTS \<delta>) q w q'"
  proof (induct w arbitrary: \<pi> q)
    case Nil
    then show ?case 
      by (metis DLTS_reach.simps(1) LTS_is_reachable.simps(1) 
                inPath.elims(2) list.simps(3) option.inject)
  next
    case (Cons a w)
    note c1 = Cons(1)
    note c2 = Cons(2)
    ...
    obtain \<sigma> \<pi>' where
    \<sigma>_\<pi>'_def: "\<pi> = \<sigma> # \<pi>'"
      by (meson inPath.elims(2) list.simps(3))
    from this obtain q''
      where "\<delta> (q,\<sigma>) = Some q''"
      using c2 by fastforce
    have pre1: "DLTS_reach \<delta> q'' \<pi>' = Some q'"
      using \<open>\<delta> (q, \<sigma>) = Some q''\<close> \<sigma>_\<pi>'_def c2 by auto
    have pre2: "inPath w \<pi>' "
      using \<sigma>_\<pi>'_def c3 by auto
    from c1[of q'' \<pi>', OF pre1 pre2] c2 c3
    show ?case 
      using \<open>\<delta> (q, \<sigma>) = Some q''\<close> \<sigma>_\<pi>'_def by auto
  qed
qed
