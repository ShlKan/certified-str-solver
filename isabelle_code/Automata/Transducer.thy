...
type_synonym 'a Tlabel = "'a set option \<times> ('a option \<Rightarrow> 'a set option)"
 

type_synonym ('q, 'a) LTTS = 
            "('q \<times> 'a Tlabel \<times> 'q) set"

record ('q, 'a) NFT =
  \<Q>T :: "'q set"           (* "The set of states" *)
  \<Sigma>T :: "'a set"           (* alphabet *)
  \<Delta>T :: " ('q, 'a) LTTS"   (* "The transition relation" *)
...
fun Edge_path :: "('q, 'a) LTTS \<Rightarrow> 'q \<Rightarrow> ('a Tlabel \<times> 'q) list \<Rightarrow> bool" where
...
fun matcht where
  "matcht (s,f) (a, b) = ((matche s a) \<and> (matche (f a) b))"


fun LTTS_reachable :: "('q, 'a) LTTS \<Rightarrow> 'q \<Rightarrow> ('a option \<times> 'a option) list 
                                                            \<Rightarrow> 'q \<Rightarrow> bool" where
  "LTTS_reachable Trans q [] q'= (q = q')" |
  "LTTS_reachable Trans q (s # w) q' = 
      (\<exists> q'' \<alpha>. (q, \<alpha>, q'') \<in> Trans \<and> matcht \<alpha> s \<and> 
                LTTS_reachable Trans q'' w q')"

lemma LTTS_reachable_concat:
    fixes q \<pi> q' \<pi>'' q''
  assumes fst_part: "LTTS_reachable Trans q \<pi> q'"
      and snd_part: "LTTS_reachable Trans q' \<pi>' q''"
    shows "LTTS_reachable Trans q (\<pi> @ \<pi>') q''"
...
definition domainT where
  "domainT \<T> = {inputE \<pi> | q \<pi> q'. q \<in> \<I>T \<T> \<and> 
                           q' \<in> \<F>T \<T> \<and> LTTS_reachable (\<Delta>T \<T>) q \<pi> q'}"

definition codomainT  :: "('q, 'a) NFT \<Rightarrow> 'a list set" where
  "codomainT \<T> = {outputE \<pi> | q \<pi> q'. q \<in> \<I>T \<T> \<and> 
                           q' \<in> \<F>T \<T> \<and> LTTS_reachable (\<Delta>T \<T>) q \<pi> q'}"

definition outputL :: "('q, 'a) NFT \<Rightarrow> ('q, 'a) NFA_rec \<Rightarrow> 'a list set" where
  "outputL \<T> \<A> = {outputE \<pi> | \<pi> q q'. q \<in> \<I>T \<T> \<and> q' \<in> \<F>T \<T> \<and> 
                                LTTS_reachable (\<Delta>T \<T>) q \<pi> q' \<and>
                                inputE \<pi> \<in> \<L> \<A>}" 

definition productT :: "('q, 'a) NFT \<Rightarrow> ('q, 'a) NFA_rec \<Rightarrow> 
  ...
  "productT \<T> \<A> F = \<lparr> 
      \<Q>e = \<Q>T \<T> \<times> \<Q> \<A>,
      \<Sigma>e = \<Sigma>T \<T>,
      \<Delta>e = {((p,p'), the (f None), (q,p')) 
              | p p' q f. p' \<in> \<Q> \<A> \<and>
                   ((p, (None, f), q) \<in> \<Delta>T \<T> \<and> (\<exists> So. f None = Some So))}
            \<union>
            { ((p,p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), (q,q')) 
                  | p p' \<sigma>1 \<sigma>2 q q' f.
               (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and> (p', \<sigma>2, q') \<in> \<Delta> \<A> \<and>
                     \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists> So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)},
      \<Delta>e' = {((p,p'), (q,p')) 
              | p p' q f. p' \<in> \<Q> \<A> \<and>
                   ((p, (None, f), q) \<in> \<Delta>T \<T> \<and> (f None = None))}
            \<union>
            { ((p,p'), (q,q')) 
                  | p p' \<sigma>1 \<sigma>2 q q' f.
               (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and> (p', \<sigma>2, q') \<in> \<Delta> \<A> \<and>
                     \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists> x \<in> (\<sigma>1 \<inter> \<sigma>2). f (Some x) = None)},
      \<I>e = \<I>T \<T> \<times> \<I> \<A>,
      \<F>e = \<F>T \<T> \<times> \<F> \<A> 
  \<rparr>"
...
lemma productT_correct:
  ...
  assumes F_ok1: "\<forall> f s. (\<forall> e \<in> s. f (Some e) = None) \<longleftrightarrow> F f s = None"
    ...
    shows "\<L>e (productT \<T> \<A> F) = outputL \<T> \<A>"
...
proof 
  {
    ...
    have fstqI: "fst q \<in> \<I>T \<T> \<and> fst q' \<in> \<F>T \<T>"
      ...
      by fastforce
    from w_def' 
    have "\<exists> \<pi>. x = outputE \<pi> \<and> LTTS_reachable (\<Delta>T \<T>) (fst q) \<pi> (fst q') 
    ...
    proof -
      {
       ...
       have "\<exists>\<pi>. [] = outputE \<pi> \<and>
                LTTS_reachable (\<Delta>T \<T>) q1 \<pi> q2 \<and> 
        ...
        proof -
          ...
          show "\<exists>\<pi>. [] = outputE \<pi> \<and> LTTS_reachable (\<Delta>T \<T>) q1 \<pi> q2 \<and> 
                    LTS_is_reachable (\<Delta> \<A>) q1' (inputE \<pi>) q2'"
          proof (cases "l = []")
            case True
            from this p1
            ...
            from this
            have "[] = outputE [] \<and> LTTS_reachable (\<Delta>T \<T>) q1 [] q2 \<and> 
          ...
          next
            ...
            have "\<exists>\<pi>. [] = outputE \<pi> \<and>
                     LTTS_reachable (\<Delta>T \<T>) (fst q3) \<pi> q2 \<and>
            ...
            from this
            have "(\<exists> f. (fst q, (None, f), fst q3) \<in> \<Delta>T \<T> \<and> f None = None \<and>
                        snd q = snd q3)\<or>
                  (\<exists> \<sigma>1 f \<sigma>2. (fst q, (Some \<sigma>1, f), fst q3) \<in> \<Delta>T \<T> \<and>
                        (snd q, \<sigma>2, snd q3) \<in> \<Delta> \<A> \<and>
                               \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> 
                                  (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))"
              unfolding productT_def
              apply simp 
            ...
            from this obtain f1 f2 \<sigma>1 \<sigma>2 
              where f12\<sigma>12_def: 
                  "((fst q, (None, f1), fst q3) \<in> \<Delta>T \<T> \<and> 
                                f1 None = None \<and> snd q = snd q3) \<or>
                   ((fst q, (Some \<sigma>1, f2), fst q3) \<in> \<Delta>T \<T> \<and> 
                        (snd q, \<sigma>2, snd q3) \<in> \<Delta> \<A> \<and>
                               \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> 
                                (\<exists> x \<in> (\<sigma>1 \<inter> \<sigma>2). f2 (Some x) = None))"
              by blast
           
            show ?thesis
            proof (cases "(fst q, (None, f1), fst q3) \<in> \<Delta>T \<T> \<and> f1 None = None 
                          \<and> snd q = snd q3")
              case True
              from \<pi>_def
              have s1: "[] = outputE ((None, None) # \<pi>)" by auto
              from True p2 LTTS_reachable.simps(2) 
                    [of "\<Delta>T \<T>" "fst q" "(None,None)" \<pi> q2] \<pi>_def
              have s2: "LTTS_reachable (\<Delta>T \<T>) q1 ((None, None) # \<pi>) q2"
                apply simp
                by force
            ...
            next
              ...
              have False': "(fst q, (Some \<sigma>1, f2), fst q3) \<in> \<Delta>T \<T> 
                    \<and> (snd q, \<sigma>2, snd q3) \<in> \<Delta> \<A> \<and>
                     \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists> x \<in> (\<sigma>1 \<inter> \<sigma>2). f2 (Some x) = None)" by auto
              from this obtain a1  where
                a1a2_def: "a1 \<in> \<sigma>1 \<inter> \<sigma>2 \<and> f2 (Some a1) = None" by force
              from \<pi>_def
              have s1: "[] = outputE ((Some a1, None) # \<pi>)"
                by auto
              from False' p2 \<pi>_def f12\<sigma>12_def LTTS_reachable.simps(2)[of "(\<Delta>T \<T>)" q1 
                        "(Some a1, None)" \<pi> q2] F_ok1
              have s2: "LTTS_reachable (\<Delta>T \<T>) q1 ((Some a1, None) # \<pi>) q2"
                apply simp               
        ...
        show "\<exists>\<pi>. [] = outputE \<pi> \<and>
             LTTS_reachable (\<Delta>T \<T>) (fst q) \<pi> (fst q') \<and>
      ...
      }
      {
        ...
        have c1: "\<exists>\<pi>. x = outputE \<pi> \<and>
                  LTTS_reachable (\<Delta>T \<T>) (fst qj) \<pi> (fst q') \<and>
        ...
        have fst_half: "\<exists> \<pi>. [] = outputE \<pi> \<and>
           LTTS_reachable (\<Delta>T \<T>) (fst q) \<pi> (fst qi) \<and>
        ...
        proof -
          ...
          proof (cases "l' = []")
            ...
            next
              ...
              have "\<exists>\<pi>. [] = outputE \<pi> \<and>
                    LTTS_reachable (\<Delta>T \<T>) (fst a') \<pi> (fst qi) \<and>
          ...
          have branch: "(\<exists> f. snd a' \<in> \<Q> \<A> \<and> (fst a, (None, f), fst a') \<in> \<Delta>T \<T> 
                 \<and> f None = None \<and> snd a = snd a') \<or> 
          ...
          proof (cases "(\<exists> f. snd a' \<in> \<Q> \<A> \<and> (fst a, (None, f), fst a') \<in> \<Delta>T \<T> 
                        \<and> f None = None \<and> snd a = snd a')")
            case True
            from this
            obtain f where
            f_def: "snd a' \<in> \<Q> \<A> \<and>
             (fst a, (None, f), fst a') \<in> \<Delta>T \<T> \<and> 
                  f None = None \<and> snd a = snd a'" by auto
            have "a = q" 
              using p1 by auto
            from this f_def \<pi>_def LTTS_reachable.simps(2)
                 [of "\<Delta>T \<T>" "fst a" "(None, None)" \<pi> "fst qi"]
            have c1: "LTTS_reachable (\<Delta>T \<T>) (fst q) ((None, None) # \<pi>) (fst qi)"
              apply simp 
              by force
...
(\<exists> x \<in> \<sigma>1 \<inter> \<sigma>2. f (Some x) = None))"
              ...
              e_def: "e \<in> \<sigma>1 \<inter> \<sigma>2 \<and> f (Some e) = None" by force
              from p1
              have qa_eq: "q = a" by force
              from this e_def \<pi>_def LTTS_reachable.simps(2)
                      [of "(\<Delta>T \<T>)" "fst q" "(Some e, None)" \<pi> "fst qi"]
                    \<sigma>12f_def F_ok1
              have s1: "LTTS_reachable (\<Delta>T \<T>) (fst q) 
        ...
        have snd_half: 
            "\<exists>\<pi>. a # x = outputE \<pi> \<and>
                   LTTS_reachable (\<Delta>T \<T>) (fst qi) \<pi> (fst q') \<and>
        ...
        proof -
          {
            ...
            (snd qi, \<sigma>2, snd qj) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (F f (\<sigma>1 \<inter> \<sigma>2) = Some \<sigma>)) \<or>
             (\<exists> f. snd qi \<in> \<Q> \<A> \<and> (fst qi, (None, f), fst qj) \<in> \<Delta>T \<T> \<and> 
                    (f None = Some \<sigma>) \<and> snd qj = snd qi)"
              unfolding productT_def
              apply simp
              by fastforce
            ...
            proof -
              ...
              next
                ...
                have "\<exists>\<pi>. [] = outputE \<pi> \<and> 
                           LTTS_reachable (\<Delta>T \<T>) (fst a1) \<pi> (fst q') \<and>
                ...
                have branch1: "(\<exists> f. snd qj \<in> \<Q> \<A> \<and> (fst qj, (None, f), fst a1) \<in> \<Delta>T \<T> \<and> 
                            f None = None \<and> snd qj = snd a1) \<or> 
             ...
             (snd qj, \<sigma>2, snd a1) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> 
                ...
                proof (cases "(\<exists> f. snd qj \<in> \<Q> \<A> \<and> (fst qj, (None, f), fst a1) \<in> \<Delta>T \<T> \<and> 
                            f None = None \<and> snd qj = snd a1)")
                  case True
                  from this obtain f where
                  f_def: "snd qj \<in> \<Q> \<A> \<and>
      (fst qj, (None, f), fst a1) \<in> \<Delta>T \<T> \<and> f None = None \<and> snd qj = snd a1"
                    by auto
                  have s1: "LTTS_reachable (\<Delta>T \<T>) 
                ...
                next
                  ...
                  \<sigma>12f_def: "(fst qj, (Some \<sigma>1, f), fst a1) \<in> \<Delta>T \<T> \<and>
                             (snd qj, \<sigma>2, snd a1) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> 
                             (\<exists> x \<in> \<sigma>1 \<inter> \<sigma>2. f (Some x) = None)" by auto
                  from this 
                  obtain e where e_def: "e \<in> \<sigma>1 \<inter> \<sigma>2 \<and> f (Some e) = None" by auto
                  have s1: "LTTS_reachable (\<Delta>T \<T>) (fst qj) 
            ...
            proof (cases "(\<exists> f. snd qi \<in> \<Q> \<A> \<and> (fst qi, (None, f), fst qj) \<in> \<Delta>T \<T> \<and> 
              ...
              from \<pi>_def f_def p1
                   LTTS_reachable.simps(2)[of "(\<Delta>T \<T>)" "fst qi" 
                                              "(None, Some a)" \<pi> "fst q'"]
              have s1: "LTTS_reachable (\<Delta>T \<T>) (fst qi) ((None, Some a)#\<pi>) (fst q')"
                by fastforce
              from \<pi>_def f_def p1 
            ...
            \<sigma>12f_def:"(fst qi, (Some \<sigma>1, f), fst qj) \<in> \<Delta>T \<T> \<and>
                      (snd qi, \<sigma>2, snd qj) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> 
                       F f (\<sigma>1 \<inter> \<sigma>2) = Some \<sigma>"
                by auto
              from F_ok2 this
              have "F f (\<sigma>1 \<inter> \<sigma>2) = 
                      Some (\<Union> {the (f (Some e)) |e. e \<in> \<sigma>1 \<inter> \<sigma>2 \<and> 
                    f (Some e) \<noteq> None})"
                by auto
              from this p1 
              obtain b where
              b_def: "b \<in> \<sigma>1 \<inter> \<sigma>2 \<and> a \<in> the (f (Some b))" 
                using F_ok1 F_ok2 by auto
              have "matcht (Some \<sigma>1, f) (Some b, Some a)"
                using F_ok1 F_ok2 by auto
              from \<pi>_def \<sigma>12f_def b_def this
                   LTTS_reachable.simps(2)
                   [of "\<Delta>T \<T>" "fst qi" "(Some b, Some a)" \<pi> "fst q'"]
              have s1: "LTTS_reachable (\<Delta>T \<T>) (fst qi) 
       ...
       LTS_is_reachable_epsilon (\<Delta>e (productT \<T> \<A> F)) (\<Delta>e' (productT \<T> \<A> F)) qj
        ...
        LTTS_reachable (\<Delta>T \<T>) (fst qk) \<pi> (fst q') \<and>
       ...
       hd (a' # l) = qj \<and> last (a' # l) = qk"
          ...
          next
            ...
            have "\<exists>\<pi>. a # x = outputE \<pi> \<and>
        LTTS_reachable (\<Delta>T \<T>) (fst b) \<pi> (fst q') \<and>
        LTS_is_reachable (\<Delta> \<A>) (snd b) (inputE \<pi>) (snd q')"
          ...
          \<pi>_def: "a # x = outputE \<pi> \<and>
            ...
            have branch: 
                  "(\<exists> f. snd qj \<in> \<Q> \<A> \<and> (fst qj, (None, f), fst b) \<in> \<Delta>T \<T> \<and> 
                          f None = None \<and> snd qj = snd b) \<or>
                   (\<exists> \<sigma>1 \<sigma>2 f.
         ...
         a' = (p, p') \<and>
         (\<exists>q. b = (q, p') \<and>
              p' \<in> \<Q> \<A> \<and> (\<exists>f. (p, (None, f), q) \<in> \<Delta>T \<T> \<and> f None = None))) \<or>
     (\<exists>p p'.
         a' = (p, p') \<and>
         (\<exists>\<sigma>1 \<sigma>2 q q'.
             ...
             (\<exists>f. (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and>
                  ...
                  \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))))) \<and>
    epsilon_reach (b # l')
     ({((p, p'), q, p') |p p' q.
       p' \<in> \<Q> \<A> \<and> (\<exists>f. (p, (None, f), q) \<in> \<Delta>T \<T> \<and> f None = None)} \<union>
      {uu.
    ...
    a' = qj \<and> (if l' = [] then b else last l') = qk"
     ...
     (\<exists>p p'.
         ...
         (\<exists>\<sigma>1 \<sigma>2 q q'.
             ...
             (\<exists>f. (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and>
                  (p', \<sigma>2, q') \<in> \<Delta> \<A> \<and>
                  \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None)))))" by auto
              from this qja'eq
              have s2: "
    (\<exists>f. (fst qj, (None, f), fst b) \<in> \<Delta>T \<T> \<and> f None = None \<and> snd qj = snd b) \<or>
    (\<exists>\<sigma>1 \<sigma>2 f.
        (fst qj, (Some \<sigma>1, f), fst b) \<in> \<Delta>T \<T> \<and>
        (snd qj, \<sigma>2, snd b) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))"
                apply simp
                by (metis fst_conv snd_conv)
              from s1 s2
    ...
    (\<exists>\<sigma>1 \<sigma>2 f.
        ...
        (snd qj, \<sigma>2, snd b) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))"
           ...
           proof (cases "(\<exists> f. snd qj \<in> \<Q> \<A> \<and> (fst qj, (None, f), fst b) \<in> \<Delta>T \<T> \<and> 
                          f None = None \<and> snd qj = snd b)")
             case True  
             from this obtain f where
             f_def: "snd qj \<in> \<Q> \<A> \<and>
      (fst qj, (None, f), fst b) \<in> \<Delta>T \<T> \<and> f None = None \<and> snd qj = snd b" by auto
             have s1: "LTTS_reachable (\<Delta>T \<T>) (fst qj) ((None, None) # \<pi>) (fst q')"
        ...
        "(fst qj, (Some \<sigma>1, f), fst b) \<in> \<Delta>T \<T> \<and>
         (snd qj, \<sigma>2, snd b) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None)"
                  by auto
                from this
                obtain c where c_def: "c \<in> \<sigma>1 \<inter> \<sigma>2 \<and> f (Some c) = None" by force
                from this
                have s1: "LTTS_reachable (\<Delta>T \<T>) (fst qj) ((Some c, None) # \<pi>) (fst q')"
                  by (metis F_ok1 IntD1 LTTS_reachable.simps(2) 
      ...
      LTS_is_reachable (\<Delta> \<A>) (snd qj) (inputE \<pi>) (snd q')" by auto
          ...
          have branch: "(\<exists> f. snd qi \<in> \<Q> \<A> \<and> 
                        (fst qi, (None, f), fst qj) \<in> \<Delta>T \<T> \<and> 
                         (f None = Some \<sigma>) \<and> snd qi = snd qj) \<or> 
                (\<exists> \<sigma>1 \<sigma>2 f.
              (fst qi, (Some \<sigma>1, f), fst qj) \<in> \<Delta>T \<T> \<and>
              (snd qi, \<sigma>2, snd qj) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> 
                  (F f (\<sigma>1 \<inter> \<sigma>2) = Some \<sigma>))"
         ...
         proof (cases "(\<exists> f. snd qi \<in> \<Q> \<A> \<and> 
                        (fst qi, (None, f), fst qj) \<in> \<Delta>T \<T> \<and> 
                         (f None = Some \<sigma>) \<and> snd qi = snd qj)")
           case True
           from True
           obtain f where 
           f_def: "snd qi \<in> \<Q> \<A> \<and> 
                        (fst qi, (None, f), fst qj) \<in> \<Delta>T \<T> \<and> 
                         (f None = Some \<sigma>) \<and> snd qi = snd qj" by auto
           from this \<pi>_def p1 LTTS_reachable.simps(2)
                              [of "\<Delta>T \<T>" "fst qi" "(None, Some aa)" \<pi> "fst q'"]
           have s1: "LTTS_reachable (\<Delta>T \<T>) 
        ...
        "(fst qi, (Some \<sigma>1, f), fst qj) \<in> \<Delta>T \<T> \<and>
         (snd qi, \<sigma>2, snd qj) \<in> \<Delta> \<A> \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> F f (\<sigma>1 \<inter> \<sigma>2) = Some \<sigma>"
             by auto
           
           from \<sigma>12f_def p1 F_ok2
           obtain c where c_def: "c \<in> \<sigma>1 \<inter> \<sigma>2 \<and> aa \<in> the (f (Some c)) "
             using disjoint_iff_not_equal inf.idem by auto
           have "matcht (Some \<sigma>1, f) (Some c, Some aa)"
             unfolding matcht.simps
             using F_ok1 F_ok2 by auto
           from this \<sigma>12f_def \<pi>_def c_def LTTS_reachable.simps(2)
                              [of "\<Delta>T \<T>" "fst qi" "(Some c, Some aa)" \<pi> "fst q'"]
           have s1: "LTTS_reachable (\<Delta>T \<T>) (fst qi) ((Some c, Some aa) # \<pi>) (fst q')"
             by fastforce
           have s2: "LTS_is_reachable (\<Delta> \<A>) (snd qi) 
     ...
     from \<pi>_def \<pi>'_def
          LTTS_reachable_concat[of "\<Delta>T \<T>" "fst q" \<pi> "fst qi" \<pi>' "fst q'"]
     have s3: "LTTS_reachable (\<Delta>T \<T>) (fst q) (\<pi> @ \<pi>') (fst q')"
       by auto
    ...
    from this obtain \<pi> where
    \<pi>_def: "x = outputE \<pi> \<and> LTTS_reachable (\<Delta>T \<T>) (fst q) \<pi> (fst q') 
                \<and> LTS_is_reachable (\<Delta> \<A>) (snd q) (inputE \<pi>) (snd q')"
      by auto
    from this w_def 
    ...
    show "\<exists>\<pi>. x = outputE \<pi> \<and>
             (\<exists>q. q \<in> \<I>T \<T> \<and>
                  (\<exists>q'. q' \<in> \<F>T \<T> \<and>
                        LTTS_reachable (\<Delta>T \<T>) q \<pi> q' \<and> inputE \<pi> \<in> \<L> \<A>))"
      by force
    }
    {
      show "{outputE \<pi> |\<pi>.
     \<exists>q. q \<in> \<I>T \<T> \<and>
         (\<exists>q'. q' \<in> \<F>T \<T> \<and> LTTS_reachable (\<Delta>T \<T>) q \<pi> q' \<and> inputE \<pi> \<in> \<L> \<A>)}
    \<subseteq> {w. \<exists>q\<in>\<I>e (productT \<T> \<A> F).
      ...
      proof -
        ...
        have "x = outputE \<pi> \<and> LTTS_reachable (\<Delta>T \<T>) q \<pi> q' \<and> 
        ...
        proof -
          ...
          assume ind_hyp: "(\<And>q q1 x. x = outputE \<pi> \<and>
           ...
           LTTS_reachable (\<Delta>T \<T>) q (a # \<pi>) q' \<and>
           LTS_is_reachable (\<Delta> \<A>) q1 (inputE (a # \<pi>)) q1' \<and> q1 \<in> \<Q> \<A>"

          from p1 have "\<exists> q'' \<alpha>. (q, \<alpha>, q'') \<in> \<Delta>T \<T> \<and> matcht \<alpha> a \<and> 
                          LTTS_reachable (\<Delta>T \<T>) q'' \<pi> q'"
            by auto
          from this obtain q'' \<alpha> where
          q''_def: "(q, \<alpha>, q'') \<in> \<Delta>T \<T> \<and> matcht \<alpha> a \<and> 
                          LTTS_reachable (\<Delta>T \<T>) q'' \<pi> q'" by auto
          from this
          have
          ...
          proof (cases "\<exists> f. \<alpha> = (None, f)")
            ...
            from this q''_def
            have c1: "(f None = None \<longrightarrow> (a = (None, None))) \<and> 
                  ((\<exists> p. f None = Some p) \<longrightarrow> (\<exists> c. a = (None, Some c) \<and>  
                                              c \<in> (the (f None))))"
              by (metis Pair_inject matche.elims(2) matcht.elims(2) 
                        option.distinct(1) option.sel)
            show ?thesis 
            proof (cases "f None = None")
              case True
              from this c1 
            ...
            next
              case False
              from this
              have "\<exists> p. f None = Some p"
                by blast
              from this obtain \<sigma> where
              \<sigma>_def: "f None = Some \<sigma>" by auto
              ...
              have ep_reach_qq1: "epsilon_reachable (q, q1) (q, q1)
                ...
                by force
              from q''_def 
              have "((q, q1), the (f None), (q'', q1)) \<in> \<Delta>e (productT \<T> \<A> F)"
                unfolding productT_def
                apply simp
          ...
          next
            ...
            next
              ...
              from this q''_def
              obtain d so where
              so_def: "f (Some si) = Some d \<and> so \<in> d \<and> snd a = Some so"
              ...
              have so_in: "so \<in> the (F f (\<sigma> \<inter> \<sigma>'))"
                 using F_ok1 all_not_in_conv by auto

              from q2\<sigma>_def q''_def
              have head_reach: "((q,q1), the (F f (\<sigma> \<inter> \<sigma>')), (q'', q2)) \<in> \<Delta>e (productT \<T> \<A> F)"
                unfolding productT_def
                apply simp
              ...
              have s2: "epsilon_reachable (q, q1) (q, q1)
                ...
                by force
              have s3: "so \<in> the (F f (\<sigma> \<inter> \<sigma>')) \<and>
                        ((q,q1), the (F f (\<sigma> \<inter> \<sigma>')), (q'', q2)) 
...
definition subtrans_comp where
    "subtrans_comp q \<alpha> f q' F fe T D1 D2 =
     FOREACHi
      (\<lambda> Tr (Dn1, Dn2). 
          Dn1 = D1 \<union> {((q,q1), (the (F f (\<alpha> \<inter> \<alpha>'))), (q',q1')) | 
                      q1 q1' \<alpha>'. (q1, \<alpha>', q1') \<in> T - Tr \<and> \<alpha> \<inter> \<alpha>' \<noteq> {} \<and> F f (\<alpha> \<inter> \<alpha>') \<noteq> None} \<and> 
          Dn2 = D2 \<union> {((q,q1), (q',q1')) | 
                      q1 q1' \<alpha>'. (q1, \<alpha>', q1') \<in> T - Tr \<and> \<alpha> \<inter> \<alpha>' \<noteq> {} \<and> fe f (\<alpha> \<inter> \<alpha>')})
      T (\<lambda> (q1, \<alpha>', q1') (D1, D2).
      (if (\<alpha> \<inter> \<alpha>' \<noteq> {}) then do {
       D1 \<leftarrow> (if (F f (\<alpha> \<inter> \<alpha>') \<noteq> None) 
                  then RETURN ({((q,q1), (the (F f (\<alpha> \<inter> \<alpha>'))), (q',q1'))} \<union> D1)
               else RETURN D1);
       D2 \<leftarrow> (if fe f (\<alpha> \<inter> \<alpha>') then 
...
lemma subtrans_comp_correct: 
  fixes q \<alpha> f q' F fe T D1 D2
  assumes finiteT: "finite T"
  shows "subtrans_comp q \<alpha> f q' F fe T D1 D2 \<le> 
          SPEC (\<lambda> (Dn1, Dn2).
              Dn1 = D1 \<union> {((q,q1), (the (F f (\<alpha> \<inter> \<alpha>'))), (q',q1')) | 
                      q1 q1' \<alpha>'. (q1, \<alpha>', q1') \<in> T \<and> \<alpha> \<inter> \<alpha>' \<noteq> {} \<and> F f (\<alpha> \<inter> \<alpha>') \<noteq> None} \<and> 
              Dn2 = D2 \<union> {((q,q1), (q',q1')) | 
                      q1 q1' \<alpha>'. (q1, \<alpha>', q1') \<in> T \<and> \<alpha> \<inter> \<alpha>' \<noteq> {} \<and> fe f (\<alpha> \<inter> \<alpha>')})"
...
definition trans_comp where
    "trans_comp F fe T1 T2 Q = 
     FOREACHi (\<lambda> T1r (Dn1, Dn2). Dn1 = {((p,p'), the (f None), (q,p')) 
              | p p' q f. p' \<in> Q \<and>
                   ((p, (None, f), q) \<in> T1 - T1r \<and> (\<exists> So. f None = Some So))}
            \<union>
            { ((p,p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), (q,q')) 
                  | p p' \<sigma>1 \<sigma>2 q q' f.
               (p, (Some \<sigma>1, f), q) \<in> T1 - T1r \<and> (p', \<sigma>2, q') \<in> T2 \<and>
                     \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists> So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)} \<and>
      Dn2 = {((p,p'), (q,p')) 
              | p p' q f. p' \<in> Q \<and>
                   ((p, (None, f), q) \<in> T1 - T1r \<and> (f None = None))}
            \<union>
            { ((p,p'), (q,q')) 
                  | p p' \<sigma>1 \<sigma>2 q q' f.
               (p, (Some \<sigma>1, f), q) \<in> T1 - T1r \<and> (p', \<sigma>2, q') \<in> T2 \<and>
       ...
       (
          if (\<alpha> = None) then 
              (if (f None = None) then 
                  ...
                  D1' \<leftarrow> copy_tran' q (the (f None)) q' Q;
                  RETURN (D1 \<union> D1', D2)
               })
          else (subtrans_comp q (the \<alpha>) f q' F fe T2 D1 D2)
        )) ({}, {})"
...
lemma trans_comp_correct:
  ...
  shows "trans_comp F fe T1 T2 Q \<le> 
          SPEC (\<lambda> (Dn1, Dn2). Dn1 = {((p,p'), the (f None), (q,p')) 
              | p p' q f. p' \<in> Q \<and>
                   ((p, (None, f), q) \<in> T1 \<and> (\<exists> So. f None = Some So))}
            \<union>
            { ((p,p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), (q,q')) 
                  | p p' \<sigma>1 \<sigma>2 q q' f.
      ...
      Dn2 = {((p,p'), (q,p')) 
              | p p' q f. p' \<in> Q \<and>
                   ((p, (None, f), q) \<in> T1 \<and> (f None = None))}
            \<union>
            { ((p,p'), (q,q')) 
                  | p p' \<sigma>1 \<sigma>2 q q' f.
               (p, (Some \<sigma>1, f), q) \<in> T1 \<and> (p', \<sigma>2, q') \<in> T2 \<and>
...
proof -
  {
    ...
    assume xin: "(x1, (None, x2a), x2b) \<in> it"
       ...
       and x2a_def: "x2a None = None"

    from x2a_def
    have eq1: "{((p, p'), the (f None), q, p') |p p' q f.
                       p' \<in> Q \<and>
                       (p, (None, f), q) \<in> T1 \<and>
                       (p, (None, f), q) \<notin> it \<and> (\<exists>So. f None = Some So)} = 
               {((p, p'), the (f None), q, p') |p p' q f.
                       p' \<in> Q \<and>
                       (p, (None, f), q) \<in> T1 \<and>
                       ((p, (None, f), q) \<in> it \<longrightarrow> f = x2a \<and> p = x1 \<and> q = x2b) \<and>
                       (\<exists>So. f None = Some So)}"
      by force
    ...
    have c2: "SPEC (\<lambda>T. T = {((x1, s), x2b, s) |s. s \<in> Q}) \<le>
             SPEC (\<lambda>D2'. {((p, p'), q, p') |p p' q.
                    ...
                    (\<exists>f. (p, (None, f), q) \<in> T1 \<and>
                         (p, (None, f), q) \<notin> it \<and> f None = None)} \<union>
                   {uu.
                    \<exists>p p' \<sigma>1 \<sigma>2 q q'.
                       uu = ((p, p'), q, q') \<and>
                       (\<exists>f. (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
                            ...
                            \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))} \<union>
                   D2' =
                   {((p, p'), q, p') |p p' q.
                    p' \<in> Q \<and>
                    (\<exists>f. (p, (None, f), q) \<in> T1 \<and>
    ...
    show "copy_tran x1 x2b Q
       \<le> SPEC (\<lambda>D2'. {((p, p'), the (f None), q, p') |p p' q f.
                       p' \<in> Q \<and>
                       (p, (None, f), q) \<in> T1 \<and>
                       (p, (None, f), q) \<notin> it \<and> (\<exists>So. f None = Some So)} \<union>
                      {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
                      ...
                      {((p, p'), the (f None), q, p') |p p' q f.
                       ...
                       (p, (None, f), q) \<in> T1 \<and>
                       ((p, (None, f), q) \<in> it \<longrightarrow> f = x2a \<and> p = x1 \<and> q = x2b) \<and>
                       (\<exists>So. f None = Some So)} \<union>
                      {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
                       (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
                       (p, (Some \<sigma>1, f), q) \<notin> it \<and>
                       (p', \<sigma>2, q') \<in> T2 \<and>
    ...
    assume xin: "(x1, (None, x2a), x2b) \<in> it"
       and itin: "it \<subseteq> T1"
       and x2a_def: "\<exists>y. x2a None = Some y"
    from x2a_def
    have eq1: "{((p, p'), q, p') |p p' q.
                       p' \<in> Q \<and>
                       (\<exists>f. (p, (None, f), q) \<in> T1 \<and>
                            (p, (None, f), q) \<notin> it \<and> f None = None)} = 
              {((p, p'), q, p') |p p' q.
                       p' \<in> Q \<and>
                       (\<exists>f. (p, (None, f), q) \<in> T1 \<and>
                            ((p, (None, f), q) \<in> it \<longrightarrow> f = x2a \<and> p = x1 \<and> q = x2b) \<and>
    ...
    from copy_tran'_correct[OF finiteQ, of x1 "the (x2a None)" x2b]
    have c1: "copy_tran' x1 (the (x2a None)) x2b Q
  \<le> SPEC (\<lambda>T. T = {((x1, s), the (x2a None), x2b, s) |s. s \<in> Q})"
      by auto

    have c2: "SPEC (\<lambda>T. T = {((x1, s), the (x2a None), x2b, s) |s. s \<in> Q}) \<le> 
        SPEC (\<lambda>D1'. {((p, p'), the (f None), q, p') |p p' q f.
                    p' \<in> Q \<and>
                    (p, (None, f), q) \<in> T1 \<and>
                    (p, (None, f), q) \<notin> it \<and> (\<exists>So. f None = Some So)} \<union>
                   {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
                    ...
                    (p, (Some \<sigma>1, f), q) \<notin> it \<and>
                    (p', \<sigma>2, q') \<in> T2 \<and>
                    \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)} \<union>
                   D1' =
                   {((p, p'), the (f None), q, p') |p p' q f.
                    p' \<in> Q \<and>
                    (p, (None, f), q) \<in> T1 \<and>
    ...
    show "copy_tran' x1 (the (x2a None)) x2b Q
       \<le> SPEC (\<lambda>D1'. {((p, p'), the (f None), q, p') |p p' q f.
                       p' \<in> Q \<and>
                       (p, (None, f), q) \<in> T1 \<and>
                       (p, (None, f), q) \<notin> it \<and> (\<exists>So. f None = Some So)} \<union>
                      {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
  ...
  }
  {
    ...
    have eq1: "{((p, p'), the (f None), q, p') |p p' q f.
     p' \<in> Q \<and>
     (p, (None, f), q) \<in> T1 \<and> (p, (None, f), q) \<notin> it \<and> (\<exists>So. f None = Some So)} = 
    {((p, p'), the (f None), q, p') |p p' q f.
     p' \<in> Q \<and>
     (p, (None, f), q) \<in> T1 \<and>
    ...
    have eq2: "{((p, p'), the (f None), q, p') |p p' q f.
     p' \<in> Q \<and>
     (p, (None, f), q) \<in> T1 \<and>
     ((p, (None, f), q) \<in> it \<longrightarrow> f = x2a \<and> None = x1b \<and> p = x1 \<and> q = x2b) \<and>
     (\<exists>So. f None = Some So)} = 
    ...
    have c1: "SPEC (\<lambda>(Dn1, Dn2).
              ...
              {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
               (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
               (p, (Some \<sigma>1, f), q) \<notin> it \<and>
               (p', \<sigma>2, q') \<in> T2 \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)} \<union>
              {((x1, q1), the (F x2a (the x1b \<inter> \<alpha>')), x2b, q1') |q1 q1' \<alpha>'.
               (q1, \<alpha>', q1') \<in> T2 \<and>
               the x1b \<inter> \<alpha>' \<noteq> {} \<and> F x2a (the x1b \<inter> \<alpha>') \<noteq> None} \<and>
              Dn2 =
              {((p, p'), q, p') |p p' q.
               p' \<in> Q \<and>
               (\<exists>f. (p, (None, f), q) \<in> T1 \<and>
              ...
              {uu.
               \<exists>q1 q1' \<alpha>'.
                  uu = ((x1, q1), x2b, q1') \<and>
                  (q1, \<alpha>', q1') \<in> T2 \<and> the x1b \<inter> \<alpha>' \<noteq> {} \<and> fe x2a (the x1b \<inter> \<alpha>')}) \<le> 
    RES {({((p, p'), the (f None), q, p') |p p' q f.
                 p' \<in> Q \<and>
                 (p, (None, f), q) \<in> T1 \<and>
                ...
                {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
                 ...
                 (p', \<sigma>2, q') \<in> T2 \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)},
                {((p, p'), q, p') |p p' q.
                 p' \<in> Q \<and>
                 (\<exists>f. (p, (None, f), q) \<in> T1 \<and>
                      ((p, (None, f), q) \<in> it \<longrightarrow>
    ...
    proof -
      {
        fix x
        assume xin: "x \<in> {((p, p'), the (f None), q, p') |p p' q f.
              p' \<in> Q \<and>
              (p, (None, f), q) \<in> T1 \<and>
              ((p, (None, f), q) \<in> it \<longrightarrow> f = x2a \<and> None = x1b \<and> p = x1 \<and> q = x2b) \<and>
              (\<exists>So. f None = Some So)} \<union>
             ...
             {((x1, q1), the (F x2a (the x1b \<inter> \<alpha>')), x2b, q1') |q1 q1' \<alpha>'.
              (q1, \<alpha>', q1') \<in> T2 \<and>
              the x1b \<inter> \<alpha>' \<noteq> {} \<and> (\<exists>y. F x2a (the x1b \<inter> \<alpha>') = Some y)}"
        show "x \<in> {((p, p'), the (f None), q, p') |p p' q f.
               p' \<in> Q \<and>
               (p, (None, f), q) \<in> T1 \<and>
               ((p, (None, f), q) \<in> it \<longrightarrow> f = x2a \<and> None = x1b \<and> p = x1 \<and> q = x2b) \<and>
               (\<exists>So. f None = Some So)} \<union>
              {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
               ...
               ((p, (Some \<sigma>1, f), q) \<in> it \<longrightarrow>
                f = x2a \<and> Some \<sigma>1 = x1b \<and> p = x1 \<and> q = x2b) \<and>
               (p', \<sigma>2, q') \<in> T2 \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)}"
        proof (cases "x \<in> {((p, p'), the (f None), q, p') |p p' q f.
              p' \<in> Q \<and>
              (p, (None, f), q) \<in> T1 \<and>
              ((p, (None, f), q) \<in> it \<longrightarrow> f = x2a \<and> None = x1b \<and> p = x1 \<and> q = x2b) \<and>
              (\<exists>So. f None = Some So)}")
          ...
          next
            case False
            from this xin
            have xin': "x \<in> {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
          (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
          (p, (Some \<sigma>1, f), q) \<notin> it \<and>
          (p', \<sigma>2, q') \<in> T2 \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)} \<union>
         {((x1, q1), the (F x2a (the x1b \<inter> \<alpha>')), x2b, q1') |q1 q1' \<alpha>'.
          (q1, \<alpha>', q1') \<in> T2 \<and>
          the x1b \<inter> \<alpha>' \<noteq> {} \<and> (\<exists>y. F x2a (the x1b \<inter> \<alpha>') = Some y)}" by force
            then show ?thesis 
            proof (cases "x \<in> {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
          (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
          (p, (Some \<sigma>1, f), q) \<notin> it \<and>
          (p', \<sigma>2, q') \<in> T2 \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)}")
              case True
            ...
            next
              case False
              from this xin'
              have xin'': "x \<in> {((x1, q1), the (F x2a (the x1b \<inter> \<alpha>')), x2b, q1') |q1 q1' \<alpha>'.
          (q1, \<alpha>', q1') \<in> T2 \<and>
          the x1b \<inter> \<alpha>' \<noteq> {} \<and> (\<exists>y. F x2a (the x1b \<inter> \<alpha>') = Some y)}"
                by force
              from x1b_def
        ...
        }
        {
          ...
          assume xin: "x \<in> {((p, p'), q, p') |p p' q.
              p' \<in> Q \<and>
              (\<exists>f. (p, (None, f), q) \<in> T1 \<and> (p, (None, f), q) \<notin> it \<and> f None = None)} \<union>
             {uu.
              \<exists>p p' \<sigma>1 \<sigma>2 q q'.
                 uu = ((p, p'), q, q') \<and>
                 (\<exists>f. (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
                      ...
                      \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))} \<union>
             {((x1, q1), x2b, q1') |q1 q1'.
              \<exists>\<alpha>'. (q1, \<alpha>', q1') \<in> T2 \<and> the x1b \<inter> \<alpha>' \<noteq> {} \<and> fe x2a (the x1b \<inter> \<alpha>')}"
          ...
          proof (cases "x \<in> {((p, p'), q, p') |p p' q.
              p' \<in> Q \<and>
              (\<exists>f. (p, (None, f), q) \<in> T1 \<and> (p, (None, f), q) \<notin> it \<and> f None = None)}")
            case True
              then show ?thesis by auto
            next
              case False
         ...
         {((x1, q1), x2b, q1') |q1 q1'.
          ...
          \<exists>\<alpha>'. (q1, \<alpha>', q1') \<in> T2 \<and> the x1b \<inter> \<alpha>' \<noteq> {} \<and> fe x2a (the x1b \<inter> \<alpha>')}"
            ...
            from x1b_def
              ...
              (q1, \<alpha>', q1') \<in> T2 \<and> the x1b \<inter> \<alpha>' \<noteq> {} \<and> fe x2a (the x1b \<inter> \<alpha>')" 
                by force
              from this fe_ok
              have q1q1'\<alpha>_def': "x = ((x1, q1), x2b, q1') \<and> 
              (q1, \<alpha>', q1') \<in> T2 \<and> the x1b \<inter> \<alpha>' \<noteq> {} \<and> 
                  (\<exists>x\<in>the x1b \<inter> \<alpha>'. x2a (Some x) = None)"
                by auto
...
from this xinit itin x1b_def y_def
            ...
            show "x \<in> {((p, p'), q, p') |p p' q.
              ...
              \<exists>p p' \<sigma>1 \<sigma>2 q q'.
                ...
                obtain p p' \<sigma>1 \<sigma>2 q q' f
                  ...
                  by fastforce
                from this fe_ok
                have fe_ok': "fe f (\<sigma>1 \<inter> \<sigma>2)" by auto 
                show ?thesis 
    ...
    from c1 subtrans_comp_correct[OF finiteT2, of x1 "the x1b" x2a x2b F fe "({((p, p'), the (f None), q, p') |p p' q f.
          p' \<in> Q \<and>
          (p, (None, f), q) \<in> T1 \<and> (p, (None, f), q) \<notin> it \<and> (\<exists>So. f None = Some So)} \<union>
         {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
          (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
          (p, (Some \<sigma>1, f), q) \<notin> it \<and>
          (p', \<sigma>2, q') \<in> T2 \<and> \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)})" "({((p, p'), q, p') |p p' q.
          p' \<in> Q \<and>
          (\<exists>f. (p, (None, f), q) \<in> T1 \<and> (p, (None, f), q) \<notin> it \<and> f None = None)} \<union>
         {uu.
          \<exists>p p' \<sigma>1 \<sigma>2 q q'.
             ...
             (\<exists>f. (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
                  ...
                  (p', \<sigma>2, q') \<in> T2 \<and>
                  \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))})"]
    show "subtrans_comp x1 (the x1b) x2a x2b F fe T2
        ({((p, p'), the (f None), q, p') |p p' q f.
          p' \<in> Q \<and>
          (p, (None, f), q) \<in> T1 \<and> (p, (None, f), q) \<notin> it \<and> (\<exists>So. f None = Some So)} \<union>
         {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
         ...
         {uu.
          \<exists>p p' \<sigma>1 \<sigma>2 q q'.
             ...
             (\<exists>f. (p, (Some \<sigma>1, f), q) \<in> T1 \<and>
                  ...
                  (p', \<sigma>2, q') \<in> T2 \<and>
                  \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))})
       \<le> RES {({((p, p'), the (f None), q, p') |p p' q f.
                 p' \<in> Q \<and>
                 (p, (None, f), q) \<in> T1 \<and>
                 ((p, (None, f), q) \<in> it \<longrightarrow>
                  f = x2a \<and> None = x1b \<and> p = x1 \<and> q = x2b) \<and>
...
definition productT_imp where
  "productT_imp \<T> \<A> F fe = do {
    ...
    (D1, D2) \<leftarrow> trans_comp F fe (\<Delta>T \<T>) (\<Delta> \<A>) (\<Q> \<A>);
...
proof -
  ...
  have c1: "SPEC (\<lambda>Q. Q = \<Q>T \<T> \<times> NFA_set_interval.NFA_rec.\<Q> \<A>) \<le>
            SPEC (\<lambda>Q. RETURN (\<Sigma>T \<T>) \<bind>
                 (\<lambda>S. trans_comp F fe (\<Delta>T \<T>) (NFA_set_interval.NFA_rec.\<Delta> \<A>)
    ...
    apply simp
  proof -
    have "trans_comp F fe (\<Delta>T \<T>) (NFA_set_interval.NFA_rec.\<Delta> \<A>)
     (NFA_set_interval.NFA_rec.\<Q> \<A>) \<bind>
    (\<lambda>(D1, D2).
        prods (\<I>T \<T>) (NFA_set_interval.NFA_rec.\<I> \<A>) \<bind>
        (\<lambda>I. prods (\<F>T \<T>) (NFA_set_interval.NFA_rec.\<F> \<A>) \<bind>
    ...
    proof -
      ...
      have "SPEC (\<lambda>(Dn1, Dn2).
            ...
            {((p, p'), the (f None), q, p') |p p' q f.
             ...
             (p, (None, f), q) \<in> \<Delta>T \<T> \<and> (\<exists>So. f None = Some So)} \<union>
            {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
            ...
            {uu.
             \<exists>p p' q f.
                uu = ((p, p'), q, p') \<and>
                p' \<in> NFA_set_interval.NFA_rec.\<Q> \<A> \<and>
                (p, (None, f), q) \<in> \<Delta>T \<T> \<and> f None = None} \<union>
            {uu.
             \<exists>p p' \<sigma>1 \<sigma>2 q q' f.
                uu = ((p, p'), q, q') \<and>
                (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and>
                (p', \<sigma>2, q') \<in> NFA_set_interval.NFA_rec.\<Delta> \<A> \<and>
                \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None)}) \<le> 
    ...
    (\<lambda>I. prods (\<F>T \<T>) (NFA_set_interval.NFA_rec.\<F> \<A>) \<bind>
         (\<lambda>Fa. RETURN
                \<lparr>\<Q>e = \<Q>T \<T> \<times> NFA_set_interval.NFA_rec.\<Q> \<A>, \<Sigma>e = \<Sigma>T \<T>,
                   \<Delta>e = {((p, p'), the (f None), q, p') |p p' q f.
                          p' \<in> NFA_set_interval.NFA_rec.\<Q> \<A> \<and>
                          (p, (None, f), q) \<in> \<Delta>T \<T> \<and> (\<exists>So. f None = Some So)} \<union>
                         {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q q' f.
                          (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and>
                   ...
                   \<Delta>e' =
                     {((p, p'), q, p') |p p' q.
                      p' \<in> NFA_set_interval.NFA_rec.\<Q> \<A> \<and>
                      (\<exists>f. (p, (None, f), q) \<in> \<Delta>T \<T> \<and> f None = None)} \<union>
                     {uu.
                      \<exists>p p' \<sigma>1 \<sigma>2 q q'.
                         uu = ((p, p'), q, q') \<and>
                         (\<exists>f. (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and>
                 ...
                 (\<lambda>Fa. RETURN
                        \<lparr>\<Q>e = \<Q>T \<T> \<times> NFA_set_interval.NFA_rec.\<Q> \<A>, \<Sigma>e = \<Sigma>T \<T>,
                           \<Delta>e = {((p, p'), the (f None), q, p') |p p' q f.
                                  ...
                                  (\<exists>So. f None = Some So)} \<union>
                                 {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p' \<sigma>1 \<sigma>2 q
                                  q' f.
                                  (p, (Some \<sigma>1, f), q) \<in> \<Delta>T \<T> \<and>
                                  (p', \<sigma>2, q') \<in> NFA_set_interval.NFA_rec.\<Delta> \<A> \<and>
                                  \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>So. F f (\<sigma>1 \<inter> \<sigma>2) = Some So)},
...
p' \<in> NFA_set_interval.NFA_rec.\<Q> \<A> \<and>
(p, (None, f), q) \<in> \<Delta>T \<T> \<and> (\<exists>So. f None = Some So)} \<union>
                                           {((p, p'), the (F f (\<sigma>1 \<inter> \<sigma>2)), q, q') |p p'
...
(p, (None, f), q) \<in> \<Delta>T \<T> \<and> f None = None)} \<union>
   ...
   \<sigma>1 \<inter> \<sigma>2 \<noteq> {} \<and> (\<exists>x\<in>\<sigma>1 \<inter> \<sigma>2. f (Some x) = None))},
    ...
    \<le> RES {\<lparr>\<Q>e = \<Q>T \<T> \<times> NFA_set_interval.NFA_rec.\<Q> \<A>, \<Sigma>e = \<Sigma>T \<T>,
      ...
      from this trans_comp_correct[of "\<Delta>T \<T>" "\<Delta> \<A>" "\<Q> \<A>" fe F, 
                         OF finite_TT finite_TA finite_Q fe_ok]
      show "trans_comp F fe (\<Delta>T \<T>) (NFA_set_interval.NFA_rec.\<Delta> \<A>)
