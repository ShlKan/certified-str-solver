...
We select a benchmark from the SMT-LIB \cite{smtlib_benchmarks} to test the effective and efficiency of CertiStrR. The benchmark contains 100 string constraints. 

CertiStr-R supports two kinds of SMT-LIB's replacement operations: (1) \texttt{str.replace s p r}, where the pattern \texttt{p} is a string and (2) \texttt{str.replace\_re s p r}, where the pattern \texttt{p} is a regular expression. 
