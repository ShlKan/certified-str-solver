...
Fig. \ref{fig-compute-nft-product} illustrates the computation of NFT products. \texttt{prods\_imp} computes the product of two sets. It is illustrated in \ref{fig-def-prods_imp}. We use the constructs from \emph{Refine\_Monadic}. The algorithm itself is intuitive: the nested two loops traverse all possible combination of states in the states.
...
definition prods_imp where
  "prods_imp Q1 Q2 =
   FOREACH {q. q |$\in$| s.|$\alpha$| Q1} (|$\lambda$| q Q. do {
...
The challenging part is how to compute the transitions sets. It is illustrated in Fig. \ref{fig-def-prods-imp}. The function \texttt{trans\_comp\_imp} traverse the transtions in the NFT and split each transtion into two cases. This function uses another two functions:

1. \texttt{copy\_tran\_impl}

2. \texttt{copy\_tran'\_impl}

\begin{figure}[ht]
	\begin{lstlisting}
definition trans_comp_imp where
  "trans_comp_imp M F fe T1 T2 Q =
    FOREACH {t. t |$\in$| dt.|$\alpha$| T1}
     (|$\lambda$| (q, (|$\alpha$|, f), q') (D1, D2). 
       (if (|$\alpha$| = None) then 
          (if ((M f) None = None) then 
            ...
            (subtrans_comp_imp M q (the |$\alpha$|) f q' F fe T2 D1 D2)
        )) (ddt.empty, ssd.empty ())"
...
\section{Refinement to Efficient Data Structure}
...
The semantics of the replacement operation can be described in two cases: (1) if there are strings in \texttt{str} that match \texttt{pattern}, then replace them with \texttt{replacement}. (2) If there are no matches, then the result is \texttt{str}.

The first case can be modeled by the SFTs. We illustrate this case by an example. Given a replacement operation $\texttt{replace}(s, $"$[0-9]+$"$, $"$\text{ITP}$"$)$, we can model this operation by an SFT.

There are three steps to model this operation:

\begin{enumerate}
  \item Generate an SNFA from the regular expression "$[0-9]+$". Adding output functions to the transitions of the SNFA that outputs empty characters to make it an SFT.
  \item Generate an SNFA from the replacement string "$"$\text{ITP}$"$". Note that a single string is also a regular expression. Adding input labels and output functions to the transitions of the SNFA to make it an SFT.
  \item Concatenate the two SFTs and adding preceding and succeeding transitions to make it accept the whole input string.
\end{enumerate}

We explain it step by step.

\noindent\emph{Step 1.}
The SNFA corresponding to the regular expression "$[0-9]+$" is shown in Figure \ref{fig-snfa-pattern}. On the left side is the SNFA and on the right side is the corresponding SFT. The function \texttt{f} is the output function that outputs empty characters. It can be defined as $\texttt{f} = \lambda x.~\texttt{None}$. The function \texttt{g} is the output function that outputs the characters in the replacement string. It can be defined as (the ascii code of $'I'$, $'T'$, $'P'$ are 73, 84, 80 respectively):
...
\noindent\emph{Step 2.}
The SNFA corresponding to the replacement string "\text{ITP}" is shown in Figure \ref{fig-snfa-replacement}. On the left side is the SNFA and on the right side is the corresponding SFT. The function \texttt{f} is the output function that outputs the characters in the replacement string. It can be defined as $\texttt{f} = \lambda x.~\texttt{Some } x$.
...
\begin{tikzpicture}[shorten >=1pt, node distance=2cm, auto]
  ...
  \path[->] 
  (q0') edge node {I/\texttt{g 1}} (q1')
  (q1') edge node {T/\texttt{g 2}} (q2')
  (q2') edge node {P/\texttt{g 3}} (q3');
\end{tikzpicture}
...
\noindent\emph{Step 3.}
The concatenation of the two SFTs is shown in Figure \ref{fig-rearranged-automata}. It contains two steps: (1) rearrange the transitions of the two SFTs to make them accept the whole input string. (2) add the preceding and succeeding transitions to make it accept the whole input string.
