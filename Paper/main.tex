...
\lstset{
  language=Isabelle,
	basicstyle=\ttfamily,
	keywordstyle=\color{blue}\bfseries,
	morekeywords={definition,if,then,else,where,record,fun,lemma,type_synonym,fixes,shows,assumes,and},
	escapeinside={|}{|},
  sensitive=true
}
...
\begin{tikzpicture}[
  ...
  layer/.style={rectangle, draw, rounded corners, fill=blue!10, minimum width=5.5cm, minimum height=1.2cm, align=center},
  arrow/.style={->, thick}
]

% Abstract Layer
\node[layer] (abstract) {%
  \textbf{Abstract Layer}\\
  FT with transition labels as \textbf{sets}
};

% Locale Layer
\node[layer, below=of abstract] (locale) {%
  \textbf{Implementation Layer}\\
  FT with transition labels as a \\boolean algebra locale
};

% Implementation Layer
\node[layer, below=of locale] (impl) {%
  \textbf{Boolean Algebra Layer}\\
  Concrete Boolean algebra implementations\\
  (e.g., intervals, bitvectors, predicates)
};

% Draw arrows with refinement labels
\draw[arrow] (abstract) -- node[right] {refinement} (locale);
\draw[arrow] (locale) -- node[right] {refinement} (impl);
...
One Important application of our formalization is in the context of string sanitization, specifically for web applications. Sanitizers are crucial for preventing security vulnerabilities such as cross-site scripting (XSS) attacks by ensuring that user inputs are properly encoded before being rendered in a web browser. Listing \ref{lst-html-encode} presents a C\# code snippet that implements an HTML encoding function, which is a sanitization operation. The function iterates through each character in the input string and encodes characters that are not safe for HTML rendering.
...
We evaluate the performance of our SFT product operation by computing the product of the HTML encoding sanitizer SFT with SFAs constructed from randomly generated Unicode user inputs. This experimental setup reflects realistic usage scenarios where diverse character sets and input patterns are encountered.
...
\begin{figure}[h] \centering
  ...
  The modeling of the aformentioned replacement operation will replace one occurrence of the substring that matches the regular expression pattern. We may also want to model left-most replacement and replace all occurrences of the substring that matches the regular expression pattern.

  One way to model left-most replacement is illustrated in the following.
