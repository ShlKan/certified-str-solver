...
A significant application of our certified transducer framework is in string constraint solving, a field that has seen intensive research development over the past decade. While our work provides formal verification guarantees, there exists a rich ecosystem of non-certified string solvers, each with distinct capabilities: Kaluza \cite{Berkeley-JavaScript} specializes in JavaScript analysis, CVC5 \cite{cvc5}, Z3-str3 \cite{Z3-str3} builds on the Z3 framework, S3P \cite{DBLP:conf/ccs/TrinhCJ14}, Ostrich \cite{pacmpl/ChenFHHHKLRW22}, and SLOTH \cite{HJLRV18}.
We believe that our work will benefit the community by providing a formal foundation for string solvers.



Beyond string theories, certification efforts in SMT solving have extended to other domains. Notable among these is the work by Shi et al. \cite{DBLP:conf/cav/ShiFLTWY20}, who developed a certified SMT solver for quantifier-free bit-vector theory, demonstrating the broader applicability of formal verification in automated reasoning.


\section{Conclusion}
\label{sec:conclusion}

In this paper, we present the first formalization of SFTs in the proof assistant Isabelle/HOL. The formalization features flexible interfaces for easily modeling different applications by supporting $\varepsilon$-transitions for both input and output, and extensive to different boolean algebras such as intervals, arithmetic operations, among others by leveraging the refinement framework. 
In order to assess the effciency and effectiveness of our formalization, we have implemented CertiStrR, an extension of the Certified String Solver CertiStr \cite{cpp/KanLRS22}, to support string replacement operations and evaluated it on a set of benchmarks from SMT-LIB 2024 \cite{smtlib_benchmarks}. The experimental results show that our formalization is efficient and effective. Even though we only illustrate the application of our formalization of SFTs in string solvers, out extensible framework on transition labels is applicable to other domains such as program verification.

In the future, we plan to extend our symbolic formalization to different variant SFTs. For example, attaching priority to transtions so that we can model the standard semantics of replacement operations in SMT-LIB. 
