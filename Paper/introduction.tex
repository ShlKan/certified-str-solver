...
\begin{enumerate}
\item We present the first formalization of symbolic finite transducers (SFTs) in the Isabelle/HOL. 
We prove the correctness of key operations between SFTs and symbolic SFAs, such as the product operation between SFTs and SFAs.
\item We leverage the refinement framework to create an extensible and efficient SFT formalization that is capable of accommodating various boolean algebras, ensuring adaptability to future transition label developments. The correctness of the refined efficient implementation is proved .
\item We develop a certified interval algebra with efficient set-theoretic operations, enabling refinement of transition labels from abstract sets to concrete intervals.
